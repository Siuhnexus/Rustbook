<main>
 <h2 id="turning-our-single-threaded-server-into-a-multithreaded-server">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#turning-our-single-threaded-server-into-a-multithreaded-server")
}}>
...
this, but first, we’ll look at the problem in action.
 ...
 <h3 id="simulating-a-slow-request-in-the-current-server-implementation">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#simulating-a-slow-request-in-the-current-server-implementation")
}}>
...
a slow request; the one we’ll implement is a thread pool.
 ...
 <h3 id="improving-throughput-with-a-thread-pool">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#improving-throughput-with-a-thread-pool")
}}>
...
server. Other options you might explore are the
  <em>
   fork/join model
  </em>
  , the
...
we’ll explore the technique we’re not going to use as a starting point.
 ...
 <p>
  <a id="code-structure-if-we-could-spawn-a-thread-for-each-request">
  </a>
 </p>
 <h4 id="spawning-a-thread-for-each-request">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#spawning-a-thread-for-each-request")
}}>
...
easier. Listing 20-11 shows the changes to make to
  <code>
   main
  </code>
...
new threads without any limit.
 ...
 <!-- Old headings. Do not remove or links may break. -->
 <p>
  <a id="creating-a-similar-interface-for-a-finite-number-of-threads">
  </a>
 </p>
 <h4 id="creating-a-finite-number-of-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-finite-number-of-threads")
}}>
...
our API. Listing 20-12 shows the hypothetical interface for a
  <code>
   ThreadPool
  </code>
...
compile, but we’ll try so the compiler can guide us in how to fix it.
 ...
 <h4 id="building-threadpool-using-compiler-driven-development">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#building-threadpool-using-compiler-driven-development")
}}>
   Building
   <code>
    ThreadPool
 ...
 <p>
  Make the changes in Listing 20-12 to
  <em>
   src/main.rs
  </em>
...
web requests.
 ...
 <p>
  ...
  that contains the following, which is the simplest
definition of a
  <code>
   ThreadPool
  </code>
...
4 as the number of elements in a collection of threads, which is what the
  ...
  type is for, as discussed in the
  <a href="#" onclick={() => {
    // @ts-ignore
...
parameter. Recall from the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch13-01-closures.html#moving-captured-values-out-of-the-closure-and-the-fn-traits")
}}>
  ...
  <!-- ignore -->
  section in Chapter 13 that we can take
closures as parameters with three different traits:
  <code>
   Fn
  </code>
  ,
...
end up doing something similar to the standard library
  <code>
   thread::spawn
  </code>
...
want.
 ...
 <h4 id="validating-the-number-of-threads-in-new">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#validating-the-number-of-threads-in-new")
}}>
...
signature to compare with the
 ...
 <h4 id="creating-space-to-store-the-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-space-to-store-the-threads")
}}>
...
which resizes itself as elements are inserted.
 ...
 <h4 id="a-worker-struct-responsible-for-sending-code-from-the-threadpool-to-a-thread">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#a-worker-struct-responsible-for-sending-code-from-the-threadpool-to-a-thread")
}}>
   A
   <code>
    Worker
...
execution. We’ll also give each worker an
  <code>
   id
  </code>
...
thread pool implementation, you’d likely want to use
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/thread/struct.Builder.html")
}}>
    <code>
     std::thread::Builder
    </code>
   ...
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/thread/struct.Builder.html#method.spawn")
}}>
    <code>
     spawn
    </code>
...
the closure that we get in
 ...
 <h4 id="sending-requests-to-threads-via-channels">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#sending-requests-to-threads-via-channels")
}}>
...
in the
  ...
  instance, as shown in Listing 20-16. The
  <code>
   Job
  </code>
  struct
...
<span class="boring">
 ...
 <p>
  <span class="caption">
   ...
   to store the
sender of a channel that transmits
   <code>
    Job
   </code>
   instances
...
workers spawn, so we’ll reference the
  <code>
   receiver
  </code>
...
share ownership of the receiver.
 ...
 <h4 id="implementing-the-execute-method">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#implementing-the-execute-method")
}}>
   Implementing the
   <code>
    execute
...
closure that
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch19-04-advanced-types.html#creating-type-synonyms-with-type-aliases")
}}>
...
longer than intended if we aren’t mindful of the lifetime of the
 ...
 <p>
  The code in Listing 20-20 that uses
  <code>
   let job = receiver.lock().unwrap().recv().unwrap();
  </code>
  works because with
