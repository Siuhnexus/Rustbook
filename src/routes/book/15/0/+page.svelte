<main>
 ...
 <h2 id="closures-anonymous-functions-that-capture-their-environment">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#closures-anonymous-functions-that-capture-their-environment")
}}>
...
customization.
 ...
 <p>
  ...
  <a id="refactoring-using-functions">
  </a>
  <a id="refactoring-with-closures-to-store-code">
  </a>
 </p>
 <h3 id="capturing-the-environment-with-closures">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#capturing-the-environment-with-closures")
}}>
...
to distribute for this limited-edition promotion. We call the
  <code>
   giveaway
  </code>
  method
...
the
  ...
  method that uses a closure. In the
  <code>
   giveaway
  </code>
  method, we get the
user preference as a parameter of type
  <code>
   Option&lt;ShirtColor&gt;
  </code>
  and call the
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/option/enum.Option.html#method.unwrap_or_else")
}}>
   <code>
    unwrap_or_else
   </code>
...
It takes one argument: a closure without any arguments that returns a value
  ...
  returns the value from within the
  <code>
   Some
  </code>
  . If the
...
body of the closure calls
  <code>
   self.most_stocked()
  </code>
  . We’re defining the closure
here, and the implementation of
  <code>
   unwrap_or_else
  </code>
...
are not able to capture their environment in this way.
 ...
 <h3 id="closure-type-inference-and-annotation">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#closure-type-inference-and-annotation")
}}>
...
some type to be inserted into the
  <code>
   Vec
  </code>
...
infers the type of
  ...
  and the return type of the closure to be
  <code>
   String
  </code>
  . Those
...
error when we next try to use a different type with the same closure.
 ...
 <h3 id="capturing-references-or-moving-ownership">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#capturing-references-or-moving-ownership")
}}>
...
if the variable name were a function name.
 ...
 <p>
  Because we can have multiple immutable references to
  <code>
   list
  </code>
  at the same time,
...
&#125;</code></pre></pre>
  <figcaption>
   ...
   to force the closure for the thread to take ownership of
   <code>
    list
   </code>
  </figcaption>
...
still only needs an immutable reference, we need to specify that
  ...
  should
be moved into the closure by putting the
  <code>
   move
  </code>
...
so the reference will be valid. Try removing the
  <code>
   move
  </code>
  keyword or using
...
get!
 ...
 <!-- Old headings. Do not remove or links may break. -->
 <p>
  <a id="storing-closures-using-generic-parameters-and-the-fn-traits">
  </a>
  <a id="limitations-of-the-cacher-implementation">
  </a>
  <a id="moving-captured-values-out-of-the-closure-and-the-fn-traits">
  </a>
 </p>
 <h3 id="moving-captured-values-out-of-closures-and-the-fn-traits">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#moving-captured-values-out-of-closures-and-the-fn-traits")
}}>
...
is moved
  ...
  the closure), the code in the body of the closure defines what
happens to the references or values when the closure is evaluated later (thus
affecting what, if anything, is moved
  <em>
   out of
  </em>
  the closure). A closure body can
...
implement one, two, or all three of these
  <code>
   Fn
  </code>
...
depending on how the closure’s body handles the values:
 ...
 <ol>
  <li>
   ...
   applies to closures that can be called once. All closures implement
...
parameter
  ...
  type is the type of the parameter named
  <code>
   f
  </code>
  , which is
the closure we provide when calling
  <code>
   unwrap_or_else
  </code>
  .
...
means
  ...
  must be able to be called once, take no arguments, and return a
  <code>
   T
  </code>
  .
...
Listing 13-7, we have a list of
  <code>
   Rectangle
  </code>
  instances and we use
  <code>
   sort_by_key
  </code>
  to order them by their
  <code>
   width
  </code>
...
compiler won’t let us use this closure with
  <code>
   sort_by_key
  </code>
  :
...
&#125;</code></pre>
  <figcaption>
   Listing 13-8: Attempting to use an
   <code>
    FnOnce
   </code>
   closure with
...
number of times
  ...
  calls the closure when sorting
  <code>
   list
  </code>
  . This code
...
environment—into the
  ...
  out of the closure by transferring ownership of
  <code>
   value
  </code>
  to
