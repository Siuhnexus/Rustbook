<main>
 <h2 id="processing-a-series-of-items-with-iterators">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#processing-a-series-of-items-with-iterators")
}}>
...
have to reimplement that logic yourself.
 ...
 <p>
  ...
  , meaning they have no effect until you call
methods that consume the iterator to use it up. For example, the code in
Listing 13-10 creates an iterator over the items in the vector
  <code>
   v1
  </code>
  by calling
...
the use of the iterator in the
  <code>
   for
  </code>
  loop. When the
...
<span class="boring">&#125;</span></code></pre></pre>
  <figcaption>
   Listing 13-11: Using an iterator in a
   <code>
    for
   </code>
   loop
...
index into, like vectors. Let’s examine how iterators do that.
 ...
 <h3 id="the-iterator-trait-and-the-next-method">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-iterator-trait-and-the-next-method")
}}>
   The
   <code>
    Iterator
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  ...
  ,
which are defining an
  <em>
   associated type
  </em>
  with this trait. We’ll talk about
...
an
  ...
  type is used in the return type of the
  <code>
   next
  </code>
...
iterator.
 ...
 <p>
  ...
  and, when iteration is over, returns
  <code>
   None
  </code>
  .
...
what values are returned from repeated calls to
  <code>
   next
  </code>
...
it is in the sequence. In other words, this code
  <em>
   consumes
  </em>
  , or uses up, the
iterator. Each call to
  <code>
   next
  </code>
...
ownership of
 ...
 <p>
  ...
  are immutable
references to the values in the vector. The
  <code>
   iter
  </code>
...
ownership of
  ...
  and returns owned values, we can call
  <code>
   into_iter
  </code>
  instead of
 ...
 <h3 id="methods-that-consume-the-iterator">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#methods-that-consume-the-iterator")
}}>
...
methods by looking in the standard library API documentation for the
  ...
  trait. Some of these methods call the
  <code>
   next
  </code>
  method in their definition, which
is why you’re required to implement the
  <code>
   next
  </code>
 ...
 <p>
  ...
  , because calling them
uses up the iterator. One example is the
  <code>
   sum
  </code>
  method, which takes ownership of
the iterator and iterates through the items by repeatedly calling
  <code>
   next
  </code>
  , thus
...
test illustrating a use of the
  <code>
   sum
  </code>
  method:
...
ownership of the iterator we call it on.
 ...
 <h3 id="methods-that-produce-other-iterators">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#methods-that-produce-other-iterators")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
  <figcaption>
   Listing 13-14: Calling the iterator adapter
   <code>
    map
   </code>
...
we need to consume the iterator here.
 ...
 <p>
  ...
  method,
which we used in Chapter 12 with
  <code>
   env::args
  </code>
...
<span class="boring">&#125;</span></code></pre></pre>
  <figcaption>
   Listing 13-15: Calling the
   <code>
    map
   </code>
   method to create a new iterator and then calling the
   <code>
    collect
   </code>
...
behavior while reusing the iteration behavior that the
  <code>
   Iterator
  </code>
  trait
...
consuming adapter methods to get results from calls to iterator adapters.
 ...
 <h3 id="using-closures-that-capture-their-environment">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-closures-that-capture-their-environment")
}}>
...
returns
 ...
 <p>
  In Listing 13-16, we use
  <code>
   filter
  </code>
  with a closure that captures the
  <code>
   shoe_size
  </code>
  variable from its environment to iterate over a collection of
  <code>
   Shoe
  </code>
  struct
...
&#125;</code></pre>
  <figcaption>
   Listing 13-16: Using the
   <code>
    filter
   </code>
   method with a closure that captures
   <code>
    shoe_size
   </code>
  </figcaption>
...
that takes ownership of the vector. Then we call
  <code>
   filter
  </code>
  to adapt that
