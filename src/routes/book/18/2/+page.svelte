<main>
 <h2 id="shared-state-concurrency">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#shared-state-concurrency")
}}>
...
for shared memory.
 ...
 <h3 id="using-mutexes-to-allow-access-to-data-from-one-thread-at-a-time">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-mutexes-to-allow-access-to-data-from-one-thread-at-a-time")
}}>
...
system and ownership rules, you can’t get locking and unlocking wrong.
 ...
 <h4 id="the-api-of-mutext">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-api-of-mutext")
}}>
   The API of
   <code>
    Mutex&lt;T&gt;
...
&#125;</code></pre></pre>
 <p>
  <span class="caption">
   Listing 16-12: Exploring the API of
   <code>
    Mutex&lt;T&gt;
   </code>
   in a
single-threaded context for simplicity
 ...
 <p>
  As with many types, we create a
  <code>
   Mutex&lt;T&gt;
  </code>
  ...
  .
To access the data inside the mutex, we use the
  <code>
   lock
  </code>
...
that we acquire a lock before using the value in
  <code>
   m
  </code>
  . The type of
  ...
  value. We can’t forget; the type system won’t let us access the inner
  <code>
   i32
  </code>
  otherwise.
...
to
  ...
  that we handled with the call to
  <code>
   unwrap
  </code>
  . The
...
to change the inner
 ...
 <h4 id="sharing-a-mutext-between-multiple-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#sharing-a-mutext-between-multiple-threads")
}}>
   Sharing a
   <code>
    Mutex&lt;T&gt;
...
thread finishes running its closure,
  <code>
   num
  </code>
...
multiple-ownership method we discussed in Chapter 15.
 ...
 <h4 id="multiple-ownership-with-multiple-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#multiple-ownership-with-multiple-threads")
}}>
...
concurrent situations.
 ...
 <p>
  ...
  is not safe to share across threads. When
  <code>
   Rc&lt;T&gt;
  </code>
  manages the reference count, it adds to the count for each call to
  <code>
   clone
  </code>
  and
...
with it. What we need is a type exactly like
  <code>
   Rc&lt;T&gt;
  </code>
...
to the reference count in a thread-safe way.
 ...
 <h4 id="atomic-reference-counting-with-arct">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#atomic-reference-counting-with-arct")
}}>
   Atomic Reference Counting with
   <code>
    Arc&lt;T&gt;
   </code>
  </a>
...
documentation for
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/sync/atomic/index.html")
}}>
   <code>
    std::sync::atomic
   </code>
...
library types aren’t implemented to use
  <code>
   Arc&lt;T&gt;
  </code>
...
parts, split those parts across threads, and then use a
  <code>
   Mutex&lt;T&gt;
  </code>
  to have each
...
atomic access to primitive types. We chose to use
  ...
  with a primitive
type for this example so we could concentrate on how
  <code>
   Mutex&lt;T&gt;
  </code>
  works.
 </p>
 <h3 id="similarities-between-refcelltrct-and-mutextarct">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#similarities-between-refcelltrct-and-mutextarct")
}}>
...
reference to the value inside it; this means
  <code>
   Mutex&lt;T&gt;
  </code>
  provides interior
mutability, as the
  ...
  family does. In the same way we used
  <code>
   RefCell&lt;T&gt;
  </code>
  in
Chapter 15 to allow us to mutate contents inside an
  <code>
   Rc&lt;T&gt;
  </code>
  , we use
...
errors when you use
  ...
  . Recall in Chapter 15 that using
  <code>
   Rc&lt;T&gt;
  </code>
  came
with the risk of creating reference cycles, where two
  <code>
   Rc&lt;T&gt;
  </code>
  values refer to
each other, causing memory leaks. Similarly,
  <code>
   Mutex&lt;T&gt;
  </code>
...
standard library API documentation for
  <code>
   Mutex&lt;T&gt;
  </code>
  and
