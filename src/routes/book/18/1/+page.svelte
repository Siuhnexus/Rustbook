<main>
 <h2 id="using-message-passing-to-transfer-data-between-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-message-passing-to-transfer-data-between-threads")
}}>
...
containing data. Here’s the idea in a slogan from
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("https://golang.org/doc/effective_go.html#concurrency")
}}>
   the Go language
documentation
  </a>
...
implementation of
  ...
  . A channel is a general programming concept by
...
to be
  ...
  if either the transmitter or receiver half is dropped.
 </p>
 <p>
...
halves to
 ...
 <p>
  ...
  . In short, the way Rust’s standard library
implements channels means a channel can have multiple
  <em>
   sending
  </em>
  ends that
produce values but only one
  <em>
   receiving
  </em>
  end that consumes those values. Imagine
...
for
  ...
  respectively, so we name our variables as such
to indicate each end. We’re using a
  <code>
   let
  </code>
...
destructures the tuples; we’ll discuss the use of patterns in
  ...
  statements
and destructuring in Chapter 18. For now, know that using a
  <code>
   let
  </code>
  statement
...
“hi”
 ...
 <p>
  ...
  to create a new thread and then using
  <code>
   move
  </code>
  to move
  ...
  into the closure so the spawned thread owns
  <code>
   tx
  </code>
  . The spawned
thread needs to own the transmitter to be able to send messages through the
channel. The transmitter has a
  <code>
   send
  </code>
...
will return an error. In this example, we’re calling
  <code>
   unwrap
  </code>
  to panic in case
...
short for
  ...
  , which will block the main thread’s execution and wait
...
an error to signal that no more values will be coming.
 ...
 <p>
  ...
  method doesn’t block, but will instead return a
  <code>
   Result&lt;T, E&gt;
  </code>
  immediately: an
  ...
  value holding a message if one is available and an
  <code>
   Err
  </code>
  value if there aren’t any messages this time. Using
  <code>
   try_recv
  </code>
  is useful if
...
changes in the compiler -->
 ...
 <h3 id="channels-and-ownership-transference">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#channels-and-ownership-transference")
}}>
...
problems: we’ll try to use a
  <code>
   val
  </code>
  value in the spawned thread
  <em>
   after
  </em>
  we’ve
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 16-9: Attempting to use
   <code>
    val
   </code>
   after we’ve sent it
down the channel
 ...
 <p>
  Here, we try to print
  <code>
   val
  </code>
  after we’ve sent it down the channel via
  <code>
   tx.send
  </code>
  .
...
after sending it; the ownership system checks that everything is okay.
 ...
 <h3 id="sending-multiple-values-and-seeing-the-receiver-waiting">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#sending-multiple-values-and-seeing-the-receiver-waiting")
}}>
...
1 second.
 ...
 <p>
  In the main thread, we’re not calling the
  <code>
   recv
  </code>
...
the spawned thread.
 ...
 <h3 id="creating-multiple-producers-by-cloning-the-transmitter">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-multiple-producers-by-cloning-the-transmitter")
}}>
