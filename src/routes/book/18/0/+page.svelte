<main>
 <h2 id="using-threads-to-run-code-simultaneously">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-threads-to-run-code-simultaneously")
}}>
...
The features that run these independent parts are called
  <em>
   threads
  </em>
  . For
...
threads. The Rust standard library uses a
  <em>
   1:1
  </em>
  model of thread implementation,
...
tradeoffs to the 1:1 model.
 ...
 <h3 id="creating-a-new-thread-with-spawn">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-new-thread-with-spawn")
}}>
...
though we told the spawned thread to print until
  <code>
   i
  </code>
...
for the operating system to switch between the threads.
 ...
 <h3 id="waiting-for-all-threads-to-finish-using-join-handles">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#waiting-for-all-threads-to-finish-using-join-handles")
}}>
...
threads run at the same time.
 ...
 <h3 id="using-move-closures-with-threads">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-move-closures-with-threads")
}}>
   Using
   <code>
    move
 ...
 <p>
  ...
  keyword with closures passed to
  <code>
   thread::spawn
  </code>
...
another. In the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch13-01-closures.html#capturing-references-or-moving-ownership")
}}>
...
we’ll concentrate more on the interaction between
  <code>
   move
  </code>
  and
...
spawned thread starts to execute,
  <code>
   v
  </code>
...
different reason. If we added
  <code>
   move
  </code>
  to the closure, we would move
  <code>
   v
  </code>
  into the
closure’s environment, and we could no longer call
  <code>
   drop
  </code>
  on it in the main
...
Listing 16-3 because Rust was being conservative and only borrowing
  <code>
   v
  </code>
  for the
thread, which meant the main thread could theoretically invalidate the spawned
thread’s reference. By telling Rust to move ownership of
  <code>
   v
  </code>
  to the spawned
