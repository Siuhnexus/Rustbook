<main>
 <h2 id="refcellt-and-the-interior-mutability-pattern">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#refcellt-and-the-interior-mutability-pattern")
}}>
   <code>
    RefCell&lt;T&gt;
   </code>
 ...
 <p>
  ...
  is a design pattern in Rust that allows you to mutate
...
compiler can’t guarantee that. The
  <code>
   unsafe
  </code>
...
interior mutability pattern.
 ...
 <h3 id="enforcing-borrowing-rules-at-runtime-with-refcellt">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#enforcing-borrowing-rules-at-runtime-with-refcellt")
}}>
...
it holds. So, what makes
  <code>
   RefCell&lt;T&gt;
  </code>
  different from a type like
  <code>
   Box&lt;T&gt;
  </code>
  ?
Recall the borrowing rules you learned in Chapter 4:
 ...
 <ul>
  <li>
   ...
   (but not both) one mutable reference
...
context. We’ll talk about how to get the functionality of
  <code>
   RefCell&lt;T&gt;
  </code>
  in a
...
examine how it’s possible.
 ...
 <h3 id="interior-mutability-a-mutable-borrow-to-an-immutable-value">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#interior-mutability-a-mutable-borrow-to-an-immutable-value")
}}>
...
value’s methods would not be able to mutate the value. Using
  <code>
   RefCell&lt;T&gt;
  </code>
  is
one way to get the ability to have interior mutability, but
  <code>
   RefCell&lt;T&gt;
  </code>
  doesn’t get around the borrowing rules completely: the borrow checker in the
compiler allows this interior mutability, and the borrowing rules are checked
at runtime instead. If you violate the rules, you’ll get a
  <code>
   panic!
  </code>
  instead of
...
an immutable value and see why that is useful.
 ...
 <h4 id="a-use-case-for-interior-mutability-mock-objects">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#a-use-case-for-interior-mutability-mock-objects")
}}>
...
This placeholder type is called a
  <em>
   test double
  </em>
...
when we’re running tests.
  ...
  are specific types of test doubles
...
called
  ...
  that takes an immutable reference to
  <code>
   self
  </code>
  and the text of the
...
is that we want to test the behavior of the
  <code>
   set_value
  </code>
  method on the
  ...
  . We can change what we pass in for the
  <code>
   value
  </code>
  parameter, but
...
able to say that if we create a
  <code>
   LimitTracker
  </code>
...
the
  ...
  trait and a particular value for
  <code>
   max
  </code>
...
create a new instance of the mock object, create a
  <code>
   LimitTracker
  </code>
  that uses the
mock object, call the
  <code>
   set_value
  </code>
  method on
...
to send. We also define an associated function
  <code>
   new
  </code>
...
take the message passed in as a parameter and store it in the
 ...
 <p>
  ...
  to something that is more than 75 percent of the
  <code>
   max
  </code>
  value. First, we
...
we assert that the list of messages that the
  <code>
   MockMessenger
  </code>
  is keeping track
...
signature of
  ...
  wouldn’t match the signature in the
  <code>
   Messenger
  </code>
  trait
...
immutable reference to the vector.
 ...
 <h4 id="keeping-track-of-borrows-at-runtime-with-refcellt">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#keeping-track-of-borrows-at-runtime-with-refcellt")
}}>
 ...
 <p>
  ...
  methods, which are part of the safe API that belongs to
  <code>
   RefCell&lt;T&gt;
  </code>
  . The
...
like the compile-time borrowing rules,
  <code>
   RefCell&lt;T&gt;
  </code>
...
would with references, the implementation of
  ...
  will panic at
runtime. Listing 15-23 shows a modification of the implementation of
  <code>
   send
  </code>
  in
Listing 15-22. We’re deliberately trying to create two mutable borrows active
for the same scope to illustrate that
  <code>
   RefCell&lt;T&gt;
  </code>
...
provide.
 ...
 <h3 id="having-multiple-owners-of-mutable-data-by-combining-rct-and-refcellt">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#having-multiple-owners-of-mutable-data-by-combining-rct-and-refcellt")
}}>
...
access to that data. If you have an
  <code>
   Rc&lt;T&gt;
  </code>
  that holds a
  ...
  , you can
get a value that can have multiple owners
  <em>
   and
  </em>
  that you can mutate!
...
list once we’ve created them. Let’s add in
  <code>
   RefCell&lt;T&gt;
  </code>
...
variable named
  ...
  have ownership of the inner
  <code>
   5
  </code>
  value rather
than transferring ownership from
  <code>
   value
  </code>
  to
...
can both refer to
 ...
 <p>
  After we’ve created the lists in
  <code>
   a
  </code>
  ,
...
automatic dereferencing feature we discussed in Chapter 5 (see the section
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch05-03-method-syntax.html#wheres-the---operator")
}}>
   “Where’s the
   <code>
    -&gt;
...
c after = Cons(RefCell &#123; value: 4 &#125;, Cons(RefCell &#123; value: 15 &#125;, Nil))
 ...
 <p>
  This technique is pretty neat! By using
  <code>
   RefCell&lt;T&gt;
  </code>
...
immutable
  ...
  value. But we can use the methods on
  <code>
   RefCell&lt;T&gt;
  </code>
  that provide
