<main>
 <h2 id="rct-the-reference-counted-smart-pointer">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#rct-the-reference-counted-smart-pointer")
}}>
   <code>
    Rc&lt;T&gt;
   </code>
...
multithreaded programs.
 ...
 <h3 id="using-rct-to-share-data">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-rct-to-share-data")
}}>
   Using
   <code>
    Rc&lt;T&gt;
...
of a third list. Conceptually, this looks similar to Figure 15-3:
 ...
 <img alt="Two lists that share ownership of a third list" class="center" src="/img/trpl15-03.svg"/>
 <p>
  <span class="caption">
   Figure 15-3: Two lists,
   <code>
    b
   </code>
   and
...
lists:
  ...
  that starts with 3 and
  <code>
   c
  </code>
  that starts with 4. Both
  <code>
   b
  </code>
  and
  ...
  lists will then continue on to the first
  <code>
   a
  </code>
...
error: could not compile `cons-list` (bin "cons-list") due to 1 previous error
 ...
 <p>
  ...
  variants own the data they hold, so when we create the
  <code>
   b
  </code>
  list,
...
in Listing 15-17, but not in every scenario.
 ...
 <p>
  ...
  , as shown in Listing 15-18. Each
  <code>
   Cons
  </code>
...
increasing the number of references from one to two and letting
  ...
  share ownership of the data in that
  <code>
   Rc&lt;List&gt;
  </code>
  . We’ll also clone
...
we call
  ...
  , the reference count to the data within the
  <code>
   Rc&lt;List&gt;
  </code>
  will
...
a new
  ...
  function and pass a reference to the
  <code>
   Rc&lt;List&gt;
  </code>
  in
...
code, we only need to consider the deep-copy clones and can disregard calls to
 ...
 <h3 id="cloning-an-rct-increases-the-reference-count">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#cloning-an-rct-increases-the-reference-count")
}}>
   Cloning an
   <code>
    Rc&lt;T&gt;
...
counts changing as we create and drop references to the
  <code>
   Rc&lt;List&gt;
  </code>
  in
 ...
 <p>
  In Listing 15-19, we’ll change
  <code>
   main
  </code>
  so it has an inner scope around list
  <code>
   c
  </code>
  ;
then we can see how the reference count changes when
  <code>
   c
  </code>
  goes out of scope.
...
also has a
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch15-06-reference-cycles.html#preventing-reference-cycles-turning-an-rct-into-a-weakt")
}}>
...
count: the implementation of the
  <code>
   Drop
  </code>
...
automatically when an
 ...
 <p>
  What we can’t see in this example is that when
  <code>
   b
  </code>
  and then
...
parts of your program for reading only. If
  <code>
   Rc&lt;T&gt;
  </code>
...
section, we’ll discuss the interior mutability pattern and the
  ...
  type that you can use in conjunction with an
  <code>
   Rc&lt;T&gt;
  </code>
  to work with this
