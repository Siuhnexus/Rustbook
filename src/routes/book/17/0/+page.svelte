<main>
 <h2 id="using-boxt-to-point-to-data-on-the-heap">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-boxt-to-point-to-data-on-the-heap")
}}>
   Using
   <code>
    Box&lt;T&gt;
...
implements a particular trait rather than being of a specific type
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#enabling-recursive-types-with-boxes")
}}>
...
known as a
  ...
  , and Chapter 17 devotes an entire section,
  <a href="#" onclick={() => {
    // @ts-ignore
...
Chapter 17!
 ...
 <h3 id="using-a-boxt-to-store-data-on-the-heap">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-a-boxt-to-store-data-on-the-heap")
}}>
   Using a
   <code>
    Box&lt;T&gt;
 ...
 <p>
  Before we discuss the heap storage use case for
  <code>
   Box&lt;T&gt;
  </code>
  , we’ll cover the
syntax and how to interact with values stored within a
 ...
 <p>
  Listing 15-1 shows how to use a box to store an
  <code>
   i32
  </code>
  value on the heap:
...
value
  ...
  , which is allocated on the heap. This program will print
  <code>
   b = 5
  </code>
  ; in
...
themselves in this way very often. Having values like a single
  <code>
   i32
  </code>
  on the
...
wouldn’t be allowed to if we didn’t have boxes.
 ...
 <p>
  ...
  can have another value of the same type as part of
...
recursive types by inserting a box in the recursive type definition.
 ...
 <p>
  As an example of a recursive type, let’s explore the
  <em>
   cons list
  </em>
  . This is a data
...
more complex situations involving recursive types.
 ...
 <h4 id="more-information-about-the-cons-list">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#more-information-about-the-cons-list")
}}>
 ...
 <p>
  ...
  is a data structure that comes from the Lisp programming language
and its dialects and is made up of nested pairs, and is the Lisp version of a
linked list. Its name comes from the
  <code>
   cons
  </code>
...
when you have a list of items in Rust,
  <code>
   Vec&lt;T&gt;
  </code>
  is a better choice to use.
Other, more complex recursive data types
  <em>
   are
  </em>
...
represent a cons list data structure of
   <code>
    i32
   </code>
   values
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 15-3: Using the
   <code>
    List
   </code>
   enum to store the list
   <code>
    1, 2, 3
   </code>
  </span>
...
Rust decides how much space it needs to store a value of a non-recursive type.
 ...
 <h4 id="computing-the-size-of-a-non-recursive-type">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#computing-the-size-of-a-non-recursive-type")
}}>
...
by looking at the
  ...
  variant, which holds a value of type
  <code>
   i32
  </code>
  and a value
...
an
  ...
  . To figure out how much memory the
  <code>
   List
  </code>
 ...
 <img alt="An infinite Cons list" class="center" src="/img/trpl15-01.svg" style="width: 50%;"/>
 <p>
  <span class="caption">
   Figure 15-1: An infinite
   <code>
    List
   </code>
 ...
 <h4 id="using-boxt-to-get-a-recursive-type-with-a-known-size">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-boxt-to-get-a-recursive-type-with-a-known-size")
}}>
   Using
   <code>
    Box&lt;T&gt;
...
storing a pointer to the value instead.
 ...
 <p>
  ...
  is a pointer, Rust always knows how much space a
  <code>
   Box&lt;T&gt;
  </code>
  needs: a pointer’s size doesn’t change based on the amount of data it’s
pointing to. This means we can put a
  <code>
   Box&lt;T&gt;
  </code>
  inside the
...
of another
  ...
  value that will be on the heap rather than inside the
  <code>
   Cons
  </code>
  variant.
...
order to have a known size
 ...
 <p>
  ...
  variant needs the size of an
  <code>
   i32
  </code>
  plus the space to store the
box’s pointer data. The
  <code>
   Nil
  </code>
...
it needs to store a
  ...
  value. Figure 15-2 shows what the
  <code>
   Cons
  </code>
  variant
...
in Chapter 17, too.
 ...
 <p>
  ...
  type is a smart pointer because it implements the
  <code>
   Deref
  </code>
  trait,
which allows
  ...
  values to be treated like references. When a
  <code>
   Box&lt;T&gt;
  </code>
  value goes out of scope, the heap data that the box is pointing to is cleaned
up as well because of the
  <code>
   Drop
  </code>
