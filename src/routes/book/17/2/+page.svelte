<main>
 <h2 id="running-code-on-cleanup-with-the-drop-trait">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#running-code-on-cleanup-with-the-drop-trait")
}}>
...
smart pointer. For example, when a
  <code>
   Box&lt;T&gt;
  </code>
...
let’s implement
 ...
 <p>
  Listing 15-14 shows a
  <code>
   CustomSmartPointer
  </code>
  struct whose only custom
functionality is that it will print
  <code>
   Dropping CustomSmartPointer!
  </code>
  when the
instance goes out of scope, to show when Rust runs the
  <code>
   drop
  </code>
  function.
...
give you a visual guide to how the
  <code>
   drop
  </code>
...
message.
 ...
 <h3 id="dropping-a-value-early-with-stdmemdrop">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#dropping-a-value-early-with-stdmemdrop")
}}>
   Dropping a Value Early with
   <code>
    std::mem::drop
   </code>
  </a>
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 15-15: Attempting to call the
   <code>
    drop
   </code>
   method from
...
error: could not compile `drop-example` (bin "drop-example") due to 1 previous error
 ...
 <p>
  ...
  . The
error message uses the term
  <em>
   destructor
  </em>
  , which is the general programming term
for a function that cleans up an instance. A
  <em>
   destructor
  </em>
  is analogous to a
...
automatically call
  ...
  error because Rust would be trying to clean up the same value
twice.
 </p>
 <p>
  We can’t disable the automatic insertion of
  <code>
   drop
  </code>
  when a value goes out of
scope, and we can’t call the
  <code>
   drop
  </code>
  method explicitly. So, if we need to force
a value to be cleaned up early, we use the
  <code>
   std::mem::drop
  </code>
  function.
...
The function is in the prelude, so we can modify
  <code>
   main
  </code>
  in Listing 15-15 to
...
references are always valid also ensures that
  <code>
   drop
  </code>
