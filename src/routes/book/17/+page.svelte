<main>
 <h1 id="smart-pointers">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#smart-pointers")
}}>
   Smart Pointers
  </a>
 </h1>
 <p>
  ...
  is a general concept for a variable that contains an address in
...
data, and have no overhead.
 ...
 <p>
  ...
  , on the other hand, are data structures that act like a
pointer but also have additional metadata and capabilities. The concept of
smart pointers isn’t unique to Rust: smart pointers originated in C++ and exist
in other languages as well. Rust has a variety of smart pointers defined in the
standard library that provide functionality beyond that provided by references.
To explore the general concept, we’ll look at a couple of different examples of
smart pointers, including a
  <em>
   reference counting
  </em>
  smart pointer type. This
...
cover the most common smart pointers in the standard library:
 ...
 <ul>
  ...
  <li>
   ...
   , a type that enforces
the borrowing rules at runtime instead of compile time
  </li>
 </ul>
 <p>
  In addition, we’ll cover the
  <em>
   interior mutability
  </em>
  pattern where an immutable
