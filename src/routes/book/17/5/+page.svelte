<main>
 <h2 id="reference-cycles-can-leak-memory">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#reference-cycles-can-leak-memory")
}}>
...
will never be dropped.
 ...
 <h3 id="creating-a-reference-cycle">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-reference-cycle")
}}>
...
fn main() &#123;&#125;</code></pre></pre>
 ...
 <p>
  We’re using another variation of the
  <code>
   List
  </code>
...
second element in the
  ...
  , meaning that
instead of having the ability to modify the
  <code>
   i32
  </code>
  value as we did in Listing
15-24, we want to modify the
  <code>
   List
  </code>
  value a
...
second item if we have a
 ...
 <p>
  In Listing 15-26, we’re adding a
  <code>
   main
  </code>
  function that uses the definitions in
Listing 15-25. This code creates a list in
  <code>
   a
  </code>
  and a list in
...
that by using the
  ...
  to change the value inside from an
  <code>
   Rc&lt;List&gt;
  </code>
  that holds a
 ...
 <p>
  When we run this code, keeping the last
  <code>
   println!
  </code>
...
variable
  ...
  , which decreases the reference count of the
  <code>
   b
  </code>
  <code>
...
reference cycles.
 ...
 <h3 id="preventing-reference-cycles-turning-an-rct-into-a-weakt">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#preventing-reference-cycles-turning-an-rct-into-a-weakt")
}}>
   Preventing Reference Cycles: Turning an
   <code>
    Rc&lt;T&gt;
   </code>
   into a
 ...
 <p>
  So far, we’ve demonstrated that calling
  <code>
   Rc::clone
  </code>
  increases the
...
and their count doesn’t affect when an
  <code>
   Rc&lt;T&gt;
  </code>
...
will be broken once the strong reference count of values involved is 0.
 ...
 <p>
  ...
  , you get a smart pointer of type
  <code>
   Weak&lt;T&gt;
  </code>
  .
...
anything with the value that a
  <code>
   Weak&lt;T&gt;
  </code>
  is pointing to, you must make sure the
value still exists. Do this by calling the
  <code>
   upgrade
  </code>
  method on a
  ...
  value has not been dropped yet and a result of
  <code>
   None
  </code>
  if the
  ...
  value has been dropped. Because
  <code>
   upgrade
  </code>
  returns an
...
item, we’ll create a tree whose items know about their children items
 ...
 <h4 id="creating-a-tree-data-structure-a-node-with-child-nodes">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-tree-data-structure-a-node-with-child-nodes")
}}>
   Creating a Tree Data Structure: a
   <code>
    Node
   </code>
   with Child Nodes
...
variables so we can access each
  <code>
   Node
  </code>
...
define the
  ...
  items to be values of type
  <code>
   Rc&lt;Node&gt;
  </code>
  . We also want to
modify which nodes are children of another node, so we have a
  <code>
   RefCell&lt;T&gt;
  </code>
  in
 ...
 <p>
  ...
  with the value 3 and no children, and another instance named
  <code>
   branch
  </code>
  with the value 5 and
  <code>
   leaf
  </code>
...
and a
 ...
 <p>
  ...
  and
doesn’t know they’re related. We want
  <code>
   leaf
  </code>
  to know that
...
parent. We’ll do that next.
 ...
 <h4 id="adding-a-reference-from-a-child-to-its-parent">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#adding-a-reference-from-a-child-to-its-parent")
}}>
...
our
  ...
  should be. We know it can’t contain an
  <code>
   Rc&lt;T&gt;
  </code>
  , because that would
create a reference cycle with
  <code>
   leaf.parent
  </code>
  pointing to
...
In Listing 15-28, we update
  <code>
   main
  </code>
  to use this new definition so the
  <code>
   leaf
  </code>
  node will have a way to refer to its parent,
  <code>
   branch
  </code>
  :
...
We still have
  ...
  reference to its parent. We use the
  <code>
   borrow_mut
  </code>
  method on the
...
the
 ...
 <p>
  When we print the parent of
  <code>
   leaf
  </code>
  again, this time we’ll get a
  <code>
   Some
  </code>
  variant
holding
  ...
  can access its parent! When we print
  <code>
   leaf
  </code>
  , we
...
cycle. We can also tell this by looking at the values we get from calling
 ...
 <h4 id="visualizing-changes-to-strong_count-and-weak_count">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#visualizing-changes-to-strong_count-and-weak_count")
}}>
   Visualizing Changes to
   <code>
    strong_count
   </code>
   and
 ...
 <p>
  ...
  into that scope. By doing so, we can see what happens when
  <code>
   branch
  </code>
  is
...
count of 0. In the inner scope, we create
  <code>
   branch
  </code>
  ...
  , at which point when we print the counts, the
  <code>
   Rc&lt;Node&gt;
  </code>
  in
  ...
  will have a strong count of 1 and a weak count of 1 (for
  <code>
   leaf.parent
  </code>
  pointing
to
  ...
  ). When we print the counts in
  <code>
   leaf
  </code>
  , we’ll see
it will have a strong count of 2, because
  <code>
   branch
  </code>
...
don’t get any memory leaks!
 ...
 <p>
  If we try to access the parent of
  <code>
   leaf
  </code>
  ...
  again. At the end of the program, the
  <code>
   Rc&lt;Node&gt;
  </code>
  in
  ...
  has a strong
count of 1 and a weak count of 0, because the variable
  <code>
   leaf
  </code>
  is now the only
reference to the
 ...
 <p>
  ...
  and their implementations of the
  <code>
   Drop
  </code>
  trait. By
...
that data can have multiple owners. The
  <code>
   RefCell&lt;T&gt;
  </code>
...
memory leaks and how to prevent them using
  <code>
   Weak&lt;T&gt;
  </code>
  .
...
smart pointers, check out
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../nomicon/index.html")
}}>
   “The Rustonomicon”
  </a>
  for more useful
