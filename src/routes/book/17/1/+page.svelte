<main>
 <h2 id="treating-smart-pointers-like-regular-references-with-the-deref-trait">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#treating-smart-pointers-like-regular-references-with-the-deref-trait")
}}>
...
type. We’ll explore how implementing the
  <code>
   Deref
  </code>
...
Rust’s
  ...
  feature and how it lets us work with either references
or smart pointers.
 </p>
...
is less important than the pointer-like behavior.
 ...
 <h3 id="following-the-pointer-to-the-value">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#following-the-pointer-to-the-value")
}}>
...
to the value it’s pointing to (hence
  <em>
   dereference
  </em>
  ) so the compiler can compare
the actual value. Once we dereference
  <code>
   y
  </code>
...
to the value it’s pointing to.
 ...
 <h3 id="using-boxt-like-a-reference">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-boxt-like-a-reference")
}}>
   Using
   <code>
    Box&lt;T&gt;
 ...
 <p>
  ...
  instead of a
reference; the dereference operator used on the
  <code>
   Box&lt;T&gt;
  </code>
  in Listing 15-7
...
&#125;</code></pre></pre>
 ...
 <p>
  ...
  pointing to a copied value of
  <code>
   x
  </code>
  rather
than a reference pointing to the value of
  <code>
   x
  </code>
  . In the last assertion, we can
use the dereference operator to follow the pointer of the
  <code>
   Box&lt;T&gt;
  </code>
  in the same
way that we did when
  <code>
   y
  </code>
...
own type.
 ...
 <h3 id="defining-our-own-smart-pointer">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#defining-our-own-smart-pointer")
}}>
   Defining Our Own Smart Pointer
  </a>
 </h3>
 <p>
  Let’s build a smart pointer similar to the
  <code>
   Box&lt;T&gt;
  </code>
...
<span class="boring">
 ...
 <p>
  ...
  and declare a generic parameter
  <code>
   T
  </code>
  , because
we want our type to hold values of any type. The
  <code>
   MyBox
  </code>
  type is a tuple struct
with one element of type
  <code>
   T
  </code>
  . The
...
implement the
 ...
 <h3 id="treating-a-type-like-a-reference-by-implementing-the-deref-trait">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#treating-a-type-like-a-reference-by-implementing-the-deref-trait")
}}>
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch10-02-traits.html#implementing-a-trait-on-a-type")
}}>
...
implementations for the trait’s required methods. The
  ...
  trait, provided
by the standard library, requires us to implement one method named
  <code>
   deref
  </code>
  that
...
<span class="boring">
 ...
 <p>
  ...
  syntax defines an associated type for the
  <code>
   Deref
  </code>
...
reference to the value we want to access with the
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch05-01-defining-structs.html#using-tuple-structs-without-named-fields-to-create-different-types")
}}>
   “Using Tuple Structs without Named Fields to Create Different
Types”
  </a>
  <!-- ignore -->
  section of Chapter 5 that
  <code>
   .0
  </code>
  accesses
the first value in a tuple struct. The
  <code>
   main
  </code>
...
plain dereference outside the parentheses in
  <code>
   *(y.deref())
  </code>
  is still necessary,
is to do with the ownership system. If the
  <code>
   deref
  </code>
...
this case or in most cases where we use the dereference operator.
 ...
 <p>
  ...
  operator is replaced with a call to the
  <code>
   deref
  </code>
  method and
...
Listing 15-9.
 ...
 <h3 id="implicit-deref-coercions-with-functions-and-methods">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#implicit-deref-coercions-with-functions-and-methods")
}}>
 ...
 <p>
  ...
  converts a reference to a type that implements the
  <code>
   Deref
  </code>
...
functions and methods, and works only on types that implement the
  <code>
   Deref
  </code>
...
can work for either references or smart pointers.
 ...
 <p>
  ...
  type we defined in
Listing 15-8 as well as the implementation of
  <code>
   Deref
  </code>
...
<span class="boring">
 ...
 <p>
  ...
  for example. Deref coercion makes it possible to call
  <code>
   hello
  </code>
  with a reference to a value of type
  <code>
   MyBox&lt;String&gt;
  </code>
...
reference to a
  ...
  value. Because we implemented the
  <code>
   Deref
  </code>
  trait
on
  ...
  in Listing 15-10, Rust can turn
  <code>
   &amp;MyBox&lt;String&gt;
  </code>
  into
  ...
  . The standard library provides an implementation of
  <code>
   Deref
  </code>
  on
...
match the parameter’s type. The number of times that
  <code>
   Deref::deref
  </code>
  needs to be
...
advantage of deref coercion!
 ...
 <h3 id="how-deref-coercion-interacts-with-mutability">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#how-deref-coercion-interacts-with-mutability")
}}>
 ...
 <p>
  ...
  operator on
immutable references, you can use the
  <code>
   DerefMut
  </code>
  trait to override the
...
immutable one. But the reverse is
  <em>
   not
  </em>
  possible: immutable references will
