<main>
 <h2 id="defining-an-enum">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#defining-an-enum")
}}>
   Defining an Enum
  </a>
 </h2>
...
value is one of a possible set of values. For example, we may want to say that
  ...
  is one of a set of possible shapes that also includes
  <code>
   Circle
  </code>
  and
...
IP address that our program will come across, we can
  <em>
   enumerate
  </em>
  all possible
...
<span class="boring">
 ...
 <h3 id="enum-values">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#enum-values")
}}>
   Enum Values
  </a>
 </h3>
...
use a double colon to separate the two. This is useful because now both values
  ...
  . We
can then, for instance, define a function that takes any
  <code>
   IpAddrKind
  </code>
  :
...
at the moment we don’t have a way to store the actual IP address
  <em>
   data
  </em>
  ; we
only know what
  ...
  it is. Given that you just learned about structs in
...
is of type
  ...
  (the enum we defined previously) and an
  <code>
   address
  </code>
  field
of type
  ...
  . We have two instances of this struct. The first is
  <code>
   home
  </code>
  ,
...
constructs an instance of the enum. That is,
  <code>
   IpAddr::V4()
  </code>
  is a function call
that takes a
  ...
  argument and returns an instance of the
  <code>
   IpAddr
  </code>
  type. We
...
IP addresses and encode which kind they are is so common that
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/net/enum.IpAddr.html")
}}>
   the standard
library has a definition we can use!
  </a>
  <!-- ignore -->
  Let’s look at how
...
defining different kinds of struct definitions, except the enum doesn’t use the
  ...
  keyword and all the variants are grouped together under the
  <code>
   Message
  </code>
...
useful:
 ...
 <h3 id="the-option-enum-and-its-advantages-over-null-values">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-option-enum-and-its-advantages-over-null-values")
}}>
   The
   <code>
    Option
...
null feature that many other languages have.
  <em>
   Null
  </em>
  is a value that means there
...
that can encode the concept of a value being present or absent. This enum is
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/option/enum.Option.html")
}}>
...
the prelude: you can use
  ...
  enum is still just a regular enum, and
  <code>
   Some(T)
  </code>
  and
...
concrete type that gets used in place of
  <code>
   T
  </code>
  makes the overall
  ...
  type
a different type. Here are some examples of using
  <code>
   Option
  </code>
  values to hold
...
we’ve specified a value inside the
  <code>
   Some
  </code>
  variant. For
  ...
  , Rust
requires us to annotate the overall
  <code>
   Option
  </code>
  type: the compiler can’t infer the
type that the corresponding
  <code>
   Some
  </code>
  ...
  value. Here, we tell Rust that we mean for
  <code>
   absent_number
  </code>
  to be of type
...
when the value is null. Everywhere that a value has a type that isn’t an
  ...
  safely assume that the value isn’t null. This was a
...
of type
  ...
  so that you can use that value? The
  <code>
   Option&lt;T&gt;
  </code>
  enum has a
...
will handle each variant. You want some code that will run only when you have a
  ...
  value, and this code is allowed to use the inner
  <code>
   T
  </code>
  . You want some
other code to run only if you have a
  <code>
   None
  </code>
