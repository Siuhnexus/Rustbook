<main>
 <h2 id="concise-control-flow-with-if-let">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#concise-control-flow-with-if-let")
}}>
...
program in Listing 6-6 that matches on an
  <code>
   Option&lt;u8&gt;
  </code>
  value in the
...
sign. It works the same way as a
  <code>
   match
  </code>
...
However, you lose the exhaustive checking that
  <code>
   match
  </code>
  enforces. Choosing
...
runs code when the value matches one pattern and then ignores all other values.
 ...
 <p>
  ...
  is the same as the block of code that would go with the
  <code>
   _
  </code>
  case in the
  ...
  enum definition in Listing 6-4, where the
  <code>
   Quarter
  </code>
  variant also held a
...
announcing the state of the quarters, we could do that with a
  <code>
   match
  </code>
...
set of enumerated values. We’ve shown how the standard library’s
  <code>
   Option&lt;T&gt;
  </code>
  type helps you use the type system to prevent errors. When enum values have
data inside them, you can use
  <code>
   match
  </code>
  or
