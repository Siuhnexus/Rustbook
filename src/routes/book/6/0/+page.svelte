<main>
 <h2 id="what-is-ownership">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#what-is-ownership")
}}>
   What Is Ownership?
  </a>
 </h2>
...
strings.
 ...
 <section aria-role="note" class="note">
  <h3 id="the-stack-and-the-heap">
   <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-stack-and-the-heap")
}}>
...
that is big enough, marks it as being in use, and returns a
   <em>
    pointer
   </em>
   , which
is the address of that location. This process is called
   <em>
    allocating on the
heap
   </em>
   and is sometimes abbreviated as just
   <em>
    allocating
   </em>
   (pushing values onto
...
is to manage heap data can help explain why it works the way it does.
 ...
 <h3 id="ownership-rules">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#ownership-rules")
}}>
   Ownership Rules
  </a>
 </h3>
...
work through the examples that illustrate them:
 ...
 <h3 id="variable-scope">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#variable-scope")
}}>
   Variable Scope
  </a>
 </h3>
...
boilerplate code.
 ...
 <p>
  As a first example of ownership, we’ll look at the
  <em>
   scope
  </em>
  of some variables. A
...
which it’s declared until the end of the current
  <em>
   scope
  </em>
  . Listing 4-1 shows a
program with comments annotating where the variable
  <code>
   s
  </code>
  would be valid.
...
<span class="boring">&#125;</span></code></pre></pre>
 ...
 <ul>
  ...
  <li>
   It remains valid until it goes
   <em>
    out of
   </em>
   scope.
...
understanding by introducing the
  <code>
   String
  </code>
  type.
 ...
 <h3 id="the-string-type">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-string-type")
}}>
   The
   <code>
    String
...
than those we covered in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch03-02-data-types.html#data-types")
}}>
   “Data Types”
  </a>
  <!-- ignore -->
...
clean up that data, and the
 ...
 <p>
  We’ll concentrate on the parts of
  <code>
   String
  </code>
  that relate to ownership. These
aspects also apply to other complex data types, whether they are provided by
the standard library or created by you. We’ll discuss
  <code>
   String
  </code>
  in more depth in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch08-02-strings.html")
}}>
   Chapter 8
  </a>
  <!-- ignore -->
...
these situations, Rust has a second string type,
  <code>
   String
  </code>
  . This type manages
data allocated on the heap and as such is able to store an amount of text that
is unknown to us at compile time. You can create a
  <code>
   String
  </code>
  from a string
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  ...
  operator allows us to namespace this particular
  <code>
   from
  </code>
  function under the
  ...
  . We’ll discuss this syntax more in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch05-03-method-syntax.html#method-syntax")
}}>
   “Method
Syntax”
  </a>
  <!-- ignore -->
  section of Chapter 5, and when we talk
about namespacing with modules in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  So, what’s the difference here? Why can
  <code>
   String
  </code>
...
cannot? The difference is in how these two types deal with memory.
 ...
 <h3 id="memory-and-allocation">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#memory-and-allocation")
}}>
...
(GC)
  ...
  , the GC keeps track of and cleans up memory that isn’t being used
...
we do it twice, that’s a bug too. We need to pair exactly one
  <code>
   allocate
  </code>
  with
...
scope, Rust calls a special function for us. This function is called
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/ops/trait.Drop.html#tymethod.drop")
}}>
   <code>
    drop
   </code>
...
we’ve allocated on the heap. Let’s explore some of those situations now.
 ...
 <p>
  <a id="ways-variables-and-data-interact-move">
  </a>
 </p>
 <h4 id="variables-and-data-interacting-with-move">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#variables-and-data-interacting-with-move")
}}>
...
string data on the heap, byte by byte." class="center" src="img/trpl04-01.svg" style="width: 50%;"/>
 </p>
 <p>
  <span class="caption">
...
stack, respectively, and both pointing to the same string data on the heap." class="center" src="img/trpl04-02.svg" style="width: 50%;"/>
 </p>
 <p>
  <span class="caption">
   ...
   that has a copy of the pointer, length, and capacity of
   <code>
    s1
   </code>
  </span>
 ...
 <p>
  ...
  look like Figure 4-3, which is what memory would
...
and each points to its own copy of string data on the heap." class="center" src="img/trpl04-03.svg" style="width: 50%;"/>
 </p>
 <p>
  <span class="caption">
...
same memory. This is known as a
  <em>
   double free
  </em>
  error and is one of the memory
...
corruption, which can potentially lead to security vulnerabilities.
 ...
 <p>
  ...
  as
no longer valid. Therefore, Rust doesn’t need to free anything when
  <code>
   s1
  </code>
  goes
out of scope. Check out what happens when you try to use
  <code>
   s1
  </code>
  after
...
access the heap data." class="center" src="img/trpl04-04.svg" style="width:
50%;"/>
 </p>
 <p>
  <span class="caption">
   Figure 4-4: Representation in memory after
   <code>
    s1
   </code>
   has been
invalidated
 ...
 <p>
  That solves our problem! With only
  <code>
   s2
  </code>
...
automatically create “deep” copies of your data. Therefore, any
 ...
 <h4 id="scope-and-assignment">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#scope-and-assignment")
}}>
...
memory being freed via the
  <code>
   drop
  </code>
  function as well. When you assign a completely
new value to an existing variable, Rust will call
  <code>
   drop
  </code>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  We initially declare a variable
  <code>
   s
  </code>
  and bind it to a
  ...
  . Then we immediately create a new
  <code>
   String
  </code>
  with the value
...
assign it to
  ...
  . At this point, nothing is referring to the original value on
the heap at all.
 </p>
 <p>
...
data (hello) grayed out because it cannot be accessed anymore." class="center" src="img/trpl04-05.svg" style="width: 50%;"/>
 </p>
 <p>
  <span class="caption">
...
at the end, it will be
 ...
 <!-- Old heading. Do not remove or links may break. -->
 <p>
  <a id="ways-variables-and-data-interact-clone">
  </a>
 </p>
 <h4 id="variables-and-data-interacting-with-clone">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#variables-and-data-interacting-with-clone")
}}>
 ...
 <p>
  ...
  want to deeply copy the heap data of the
  <code>
   String
  </code>
  , not just the
stack data, we can use a common method called
  <code>
   clone
  </code>
...
different is going on.
 ...
 <h4 id="stack-only-data-copy">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#stack-only-data-copy")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  ...
  is still valid and wasn’t moved into
  <code>
   y
  </code>
  .
...
to make. That means there’s no reason we would want to prevent
  <code>
   x
  </code>
  from being
valid after we create the variable
  <code>
   y
  </code>
...
when the value goes out of scope and we add the
  <code>
   Copy
  </code>
  annotation to that type,
we’ll get a compile-time error. To learn about how to add the
  <code>
   Copy
  </code>
  annotation
to your type to implement the trait, see
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("appendix-03-derivable-traits.html")
}}>
   “Derivable
Traits”
  </a>
...
implement
 ...
 <h3 id="ownership-and-functions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#ownership-and-functions")
}}>
...
the ownership rules prevent you from doing so.
 ...
 <h3 id="return-values-and-scope">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#return-values-and-scope")
}}>
...
heap goes out of scope, the value will be cleaned up by
  <code>
   drop
  </code>
  unless ownership
