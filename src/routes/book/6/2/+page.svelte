<main>
 <h2 id="the-slice-type">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-slice-type")
}}>
   The Slice Type
  </a>
 </h2>
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch08-00-common-collections.html")
}}>
   collection
  </a>
  rather than the whole collection. A
...
way to talk about
  ...
  of a string. However, we could return the index of the
...
<span class="boring">
  ...
  <figcaption>
   ...
   function that returns a byte index value into the
   <code>
    String
   </code>
   parameter
...
a value is a space, we’ll convert our
  <code>
   String
  </code>
...
<span class="boring">        if item == b' ' &#123;
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch13-02-iterators.html")
}}>
   Chapter 13
  </a>
  <!-- ignore -->
...
destructure that tuple. We’ll be discussing patterns more in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch06-02-match.html#patterns-that-bind-to-values")
}}>
   Chapter
6
  </a>
  ...
  loop, we specify a pattern that has
  <code>
   i
  </code>
  for the index in the tuple and
  <code>
   &amp;item
  </code>
...
Otherwise, we return the length of the string by using
  <code>
   s.len()
  </code>
  .
...
string, but there’s a problem. We’re returning a
  <code>
   usize
  </code>
  on its own, but it’s
only a meaningful number in the context of the
  <code>
   &amp;String
  </code>
  . In other words,
because it’s a separate value from the
  <code>
   String
  </code>
...
&#125;</code></pre></pre>
  <figcaption>
   ...
   function and then changing the
   <code>
    String
   </code>
   contents
 ...
 <p>
  ...
  isn’t connected to the state of
  <code>
   s
  </code>
  at all,
...
because the contents of
 ...
 <p>
  Having to worry about the index in
  <code>
   word
  </code>
...
we write a
 ...
 <p>
  Now we’re tracking a starting
  <em>
   and
  </em>
  an ending index, and we have even more
...
to be kept in sync.
 ...
 <h3 id="string-slices">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#string-slices")
}}>
   String Slices
  </a>
 </h3>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  Rather than a reference to the entire
  <code>
   String
  </code>
  ,
...
portion of the
  ...
  , specified in the extra
  <code>
   [0..5]
  </code>
  bit. We create slices
using a range within brackets by specifying
  <code>
...
where
  ...
  is the first position in the slice and
  <code>
   ending_index
  </code>
  is
...
has a length value of 5 and points to byte 6 of the heap data table.' class="center" src="img/trpl04-07.svg" style="width: 50%;"/>
 </p>
 <p>
  <span class="caption">
...
more thorough discussion of UTF-8 handling is in the
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch08-02-strings.html#storing-utf-8-encoded-text-with-strings")
}}>
...
Text with Strings”
 ...
 <p>
  ...
  to return a
slice. The type that signifies “string slice” is written as
  <code>
   &amp;str
  </code>
  :
...
compiler will ensure the references into the
  <code>
   String
  </code>
...
our code much sooner. Using the slice version of
  <code>
   first_word
  </code>
  will throw a
...
truncate the
  ...
  , it needs to get a mutable reference. The
  <code>
   println!
  </code>
  after the call to
...
reference in
  ...
  and the immutable reference in
  <code>
   word
  </code>
...
but it has also eliminated an entire class of errors at compile time!
 ...
 <p>
  <a id="string-literals-are-slices">
  </a>
 </p>
 <h4 id="string-literals-as-slices">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#string-literals-as-slices")
}}>
...
the binary. This is also why string literals are immutable;
  <code>
   &amp;str
  </code>
  is an
immutable reference.
 ...
 <h4 id="string-slices-as-parameters">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#string-slices-as-parameters")
}}>
...
<span class="boring">    let bytes = s.as_bytes();
  ...
  <figcaption>
   Listing 4-9: Improving the
   <code>
    first_word
   </code>
   function by using a string slice for the type of the
   <code>
    s
   </code>
   parameter
...
can pass a slice of the
  ...
  . This
flexibility takes advantage of
  <em>
   deref coercions
  </em>
  , a feature we will cover in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch15-02-deref.html#implicit-deref-coercions-with-functions-and-methods")
}}>
...
&#125;</code></pre></pre>
 ...
 <h3 id="other-slices">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#other-slices")
}}>
   Other Slices
  </a>
 </h3>
