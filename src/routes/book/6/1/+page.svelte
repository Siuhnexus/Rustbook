<main>
 <h2 id="references-and-borrowing">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#references-and-borrowing")
}}>
...
A
  ...
  is like a pointer in that it’s an address we can follow to access
...
function return value is gone. Second, note that we pass
  ...
  . These ampersands represent
  <em>
   references
  </em>
  , and they allow you to refer
to some value without taking ownership of it. Figure 4-6 depicts this concept.
 </p>
 <p>
  <img alt="Three tables: the table for s contains only a pointer to the table
for s1. The table for s1 contains the stack data for s1 and points to the
string data on the heap." class="center" src="img/trpl04-06.svg"/>
 </p>
 <p>
  <span class="caption">
   Figure 4-6: A diagram of
   <code>
    &amp;String s
   </code>
   pointing at
...
<span class="boring">
 ...
 <p>
  ...
  syntax lets us create a reference that
  <em>
   refers
  </em>
  to the value of
...
&#125; // Here, s goes out of scope. But because it does not have ownership of what
 ...
 <p>
  The scope in which the variable
  <code>
   s
  </code>
...
ownership.
 ...
 <p>
  We call the action of creating a reference
  <em>
   borrowing
  </em>
...
allowed to modify something we have a reference to.
 ...
 <h3 id="mutable-references">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#mutable-references")
}}>
   Mutable References
  </a>
 </h3>
...
&#125;</code></pre></pre>
 ...
 <p>
  ...
  . Then we create a mutable reference with
  <code>
   &amp;mut s
  </code>
  where we call the
...
attempts to create two mutable references to
  <code>
   s
  </code>
  will fail:
...
error: could not compile `ownership` (bin "ownership") due to 1 previous error
 ...
 <p>
  ...
  as
mutable more than once at a time. The first mutable borrow is in
  <code>
   r1
  </code>
  and must
last until it’s used in the
  <code>
   println!
  </code>
...
prevent data races at compile time. A
  <em>
   data race
  </em>
  is similar to a race
...
multiple mutable references, just not
  <em>
   simultaneous
  </em>
  ones:
...
compile because the last usage of the immutable references, the
  <code>
   println!
  </code>
  ,
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  The scopes of the immutable references
  <code>
   r1
  </code>
  and
...
have to track down why your data isn’t what you thought it was.
 ...
 <h3 id="dangling-references">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#dangling-references")
}}>
   Dangling References
  </a>
 </h3>
...
pointer
  ...
  —a pointer that references a location in memory that may have been
...
this reference would be pointing to an invalid
  <code>
   String
  </code>
  . That’s no good! Rust
won’t let us do this.
 ...
 <p>
  The solution here is to return the
  <code>
   String
  </code>
  directly:
...
deallocated.
 ...
 <h3 id="the-rules-of-references">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-rules-of-references")
}}>
