<main>
 <h2 id="validating-references-with-lifetimes">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#validating-references-with-lifetimes")
}}>
...
references are valid as long as we need them to be.
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch04-02-references-and-borrowing.html#references-and-borrowing")
}}>
   “References and
Borrowing”
  </a>
...
that every reference in Rust has a
  <em>
   lifetime
  </em>
  , which is the scope for which
...
lifetime syntax so you can get comfortable with the concept.
 ...
 <h3 id="preventing-dangling-references-with-lifetimes">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#preventing-dangling-references-with-lifetimes")
}}>
...
inner scope declares a variable named
  <code>
   x
  </code>
  ...
  . Inside
the inner scope, we attempt to set the value of
  <code>
   r
  </code>
  as a reference to
  ...
  . Then
the inner scope ends, and we attempt to print the value in
  <code>
   r
  </code>
  . This code won’t
compile because the value that
  <code>
   r
  </code>
...
error: could not compile `chapter10` (bin "chapter10") due to 1 previous error
 ...
 <p>
  The error message says that the variable
  <code>
   x
  </code>
...
that it “lives longer.” If Rust allowed this code to work,
  <code>
   r
  </code>
  would be
referencing memory that was deallocated when
  <code>
   x
  </code>
  went out of scope, and
anything we tried to do with
  <code>
   r
  </code>
...
determine that this code is invalid? It uses a borrow checker.
 ...
 <h3 id="the-borrow-checker">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-borrow-checker")
}}>
   The Borrow Checker
  </a>
 </h3>
 <p>
  The Rust compiler has a
  <em>
   borrow checker
  </em>
  that compares scopes to determine
...
&#125;                         // ---------+</code></pre>
 <p>
  <span class="caption">
   Listing 10-17: Annotations of the lifetimes of
   <code>
    r
   </code>
   and
...
longer lifetime than the reference
 ...
 <p>
  ...
  , which in this case is larger than
  <code>
   'a
  </code>
  . This
means
  ...
  because Rust knows that the reference in
  <code>
   r
  </code>
  will
always be valid while
  <code>
   x
  </code>
  is valid.
...
lifetimes of parameters and return values in the context of functions.
 ...
 <h3 id="generic-lifetimes-in-functions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#generic-lifetimes-in-functions")
}}>
...
rather than strings, because we don’t want the
  <code>
   longest
  </code>
  function to take
ownership of its parameters. Refer to the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch04-03-slices.html#string-slices-as-parameters")
}}>
   “String Slices as
Parameters”
  </a>
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 10-20: An implementation of the
   <code>
    longest
   </code>
...
on it because Rust can’t tell whether the reference being returned refers to
  ...
  . Actually, we don’t know either, because the
  <code>
   if
  </code>
  block in the body
of this function returns a reference to
  <code>
   x
  </code>
  and the
...
be passed into this function, so we don’t know whether the
  <code>
   if
  </code>
  case or the
...
doesn’t know how the lifetimes of
  <code>
   x
  </code>
  and
...
perform its analysis.
 ...
 <h3 id="lifetime-annotation-syntax">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#lifetime-annotation-syntax")
}}>
...
and very short, like generic types. Most people use the name
  <code>
   'a
  </code>
  for the first
...
reference, using a space to separate the annotation from the reference’s type.
 ...
 <p>
  Here are some examples: a reference to an
  <code>
   i32
  </code>
...
reference to an
  ...
  that has a lifetime parameter named
  <code>
   'a
  </code>
  , and a mutable
...
relate to each other in the context of the
  <code>
   longest
  </code>
  function.
 </p>
 <h3 id="lifetime-annotations-in-function-signatures">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#lifetime-annotations-in-function-signatures")
}}>
...
generic
  ...
  parameters inside angle brackets between the function name
and the parameter list, just as we did with generic
  <em>
   type
  </em>
  parameters.
...
lifetime that is equal to the smaller of the lifetimes of
  ...
  . Because
we’ve annotated the returned reference with the same lifetime parameter
  <code>
   'a
  </code>
  ,
...
references to
 ...
 <p>
  ...
  is valid until the end of the outer scope,
  <code>
   string2
  </code>
  is valid until the end of the inner scope, and
  <code>
   result
  </code>
...
assignment of the value to the
  <code>
   result
  </code>
...
values using the same lifetime parameter
  <code>
   'a
  </code>
  .
...
Because
  ...
  has not gone out of scope yet, a reference to
  <code>
   string1
  </code>
  will
still be valid for the
  <code>
   println!
  </code>
...
borrow checker before you compile; then check to see if you’re right!
 ...
 <h3 id="thinking-in-terms-of-lifetimes">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#thinking-in-terms-of-lifetimes")
}}>
...
string slice, we wouldn’t need to specify a lifetime on the
  <code>
   y
  </code>
  parameter. The
...
&#125;</code></pre></pre>
 <p>
  ...
  and the return
type, but not for the parameter
  <code>
   y
  </code>
  ...
  does not have
any relationship with the lifetime of
  <code>
   x
  </code>
  or the return value.
...
the reference returned does
  <em>
   not
  </em>
  refer to one of the parameters, it must refer
to a value created within this function. However, this would be a dangling
reference because the value will go out of scope at the end of the function.
Consider this attempted implementation of the
  <code>
   longest
  </code>
  function that won’t
...
of the
  ...
  function. We’re also trying to return a reference to
  <code>
   result
  </code>
...
would create dangling pointers or otherwise violate memory safety.
 ...
 <h3 id="lifetime-annotations-in-struct-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#lifetime-annotations-in-struct-definitions")
}}>
...
annotation means an instance of
  <code>
   ImportantExcerpt
  </code>
...
it holds in its
 ...
 <p>
  ...
  function here creates an instance of the
  <code>
   ImportantExcerpt
  </code>
  struct
that holds a reference to the first sentence of the
  <code>
   String
  </code>
  owned by the
variable
  ...
  instance is created. In addition,
  <code>
   novel
  </code>
...
the
 ...
 <h3 id="lifetime-elision">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#lifetime-elision")
}}>
   Lifetime Elision
  </a>
 </h3>
...
even fewer lifetime annotations might be required.
 ...
 <p>
  ...
  . These aren’t rules for programmers to follow; they’re
...
by adding the lifetime annotations.
 ...
 <p>
  Lifetimes on function or method parameters are called
  <em>
   input lifetimes
  </em>
  , and
lifetimes on return values are called
  <em>
   output lifetimes
  </em>
  .
...
lifetimes of the references in the signature of the
  <code>
   first_word
  </code>
  function in
...
gets its own lifetime. We’ll call it
  <code>
   'a
  </code>
...
input lifetime. The third rule doesn’t apply either, because
  <code>
   longest
  </code>
  is a
function rather than a method, so none of the parameters are
  <code>
   self
  </code>
  . After
...
annotate lifetimes in method signatures very often.
 ...
 <h3 id="lifetime-annotations-in-method-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#lifetime-annotations-in-method-definitions")
}}>
...
struct named
 ...
 <p>
  First we’ll use a method named
  <code>
   level
  </code>
...
<span class="boring">
 ...
 <p>
  The lifetime parameter declaration after
  <code>
   impl
  </code>
...
one of the parameters is
  ...
  , the return type gets the lifetime of
  <code>
   &amp;self
  </code>
  ,
and all lifetimes have been accounted for.
 ...
 <h3 id="the-static-lifetime">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-static-lifetime")
}}>
   The Static Lifetime
  </a>
 </h3>
...
affected reference
  ...
  live for the entire duration of the program. All
...
is to fix those problems, not to specify the
 ...
 <h2 id="generic-type-parameters-trait-bounds-and-lifetimes-together">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#generic-type-parameters-trait-bounds-and-lifetimes-together")
}}>
...
two string slices. But now it has an extra parameter named
  <code>
   ann
  </code>
  of the generic
type
  ...
  , which can be filled in by any type that implements the
  <code>
   Display
  </code>
  trait as specified by the
  <code>
   where
  </code>
...
the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../reference/index.html")
}}>
   Rust Reference
  </a>
  . But next, you’ll learn how to write tests in
