<main>
 <h1 id="generic-types-traits-and-lifetimes">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#generic-types-traits-and-lifetimes")
}}>
...
of concepts. In Rust, one such tool is
  <em>
   generics
  </em>
  : abstract stand-ins for
...
used generics in Chapter 6 with
  <code>
   Option&lt;T&gt;
  </code>
  , in Chapter 8 with
...
generic types in struct and enum definitions.
 ...
 <p>
  Then you’ll learn how to use
  <em>
   traits
  </em>
  to define behavior in a generic way. You
...
only those types that have a particular behavior, as opposed to just any type.
 ...
 <p>
  ...
  : a variety of generics that give the
...
help.
 ...
 <h2 id="removing-duplication-by-extracting-a-function">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#removing-duplication-by-extracting-a-function")
}}>
...
numbers
 ...
 <p>
  We store a list of integers in the variable
  <code>
   number_list
  </code>
  and place a reference
to the first number in the list in a variable named
  <code>
   largest
  </code>
  . We then iterate
...
slice of
  ...
  values and one that finds the largest item in a slice of
  <code>
   char
  </code>
