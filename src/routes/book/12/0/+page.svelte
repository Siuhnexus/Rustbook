<main>
 <h2 id="generic-data-types">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#generic-data-types")
}}>
   Generic Data Types
  </a>
 </h2>
...
generics. Then we’ll discuss how generics affect code performance.
 ...
 <h3 id="in-function-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#in-function-definitions")
}}>
...
Rust’s type-naming convention is UpperCamelCase. Short for
  <em>
   type
  </em>
  ,
...
to declare the type parameter name before we use it. To define the generic
 ...
 <p>
  We read this definition as: the function
  <code>
   largest
  </code>
...
states that the body of
  ...
  won’t work for all possible types that
  <code>
   T
  </code>
  could be. Because we want to compare values of type
  <code>
   T
  </code>
  in the body, we can
...
suggestion, we restrict the types valid for
  <code>
   T
  </code>
...
implements
 ...
 <h3 id="in-struct-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#in-struct-definitions")
}}>
...
the fields
  ...
  that same type, whatever that type may be. If
...
&#125;</code></pre>
 <p>
  <span class="caption">
   ...
   must be the same
type because both have the same generic data type
   <code>
    T
   </code>
   .
 ...
 <p>
  In this example, when we assign the integer value
  <code>
   5
  </code>
  to
  ...
  , we let the
compiler know that the generic type
  <code>
   T
  </code>
...
Listing 10-8, we change the definition of
  <code>
   Point
  </code>
...
pieces.
 ...
 <h3 id="in-enum-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#in-enum-definitions")
}}>
   In Enum Definitions
  </a>
 </h3>
...
variants. Let’s take another look at the
  <code>
   Option&lt;T&gt;
  </code>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  ...
  enum is generic over two types,
  <code>
   T
  </code>
  and
  ...
  . This definition makes it convenient to use the
  <code>
   Result
  </code>
  enum anywhere we
have an operation that might succeed (return a value of some type
  <code>
   T
  </code>
  ) or fail
(return an error of some type
  <code>
   E
  </code>
...
file in Listing 9-3, where
  ...
  when
the file was opened successfully and
  <code>
   E
  </code>
  was filled in with the type
...
avoid duplication by using generic types instead.
 ...
 <h3 id="in-method-definitions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#in-method-definitions")
}}>
...
generic types in their definitions too. Listing 10-9 shows the
  ...
  struct we defined in Listing 10-6 with a method named
  <code>
   x
  </code>
  implemented on it.
...
&#125;</code></pre></pre>
 <p>
  <span class="caption">
   Listing 10-9: Implementing a method named
   <code>
    x
   </code>
   on the
   ...
   struct that will return a reference to the
   <code>
    x
   </code>
   field of type
 ...
 <p>
  Here, we’ve defined a method named
  <code>
   x
  </code>
  on
...
to the data in the field
 ...
 <p>
  ...
  to specify
that we’re implementing methods on the type
  <code>
   Point&lt;T&gt;
  </code>
  . By declaring
...
conventional. Methods written within an
  <code>
   impl
  </code>
...
type. We could, for example, implement methods only on
  <code>
   Point&lt;f32&gt;
  </code>
  instances
...
use the concrete type
  ...
  , meaning we don’t declare any types after
  <code>
   impl
  </code>
  .
...
struct with a particular concrete type for the generic type parameter
   <code>
    T
   </code>
  </span>
...
types
  ...
  method
signature to make the example clearer. The method creates a new
  <code>
   Point
  </code>
  instance with the
...
definition. Here, the generic parameters
  <code>
   X1
  </code>
  and
...
method.
 ...
 <h3 id="performance-of-code-using-generics">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#performance-of-code-using-generics")
}}>
...
process, the compiler reads the values that have been used in
  ...
  instances and identifies two kinds of
  <code>
   Option&lt;T&gt;
  </code>
  : one is
