<main>
 <h2 id="advanced-traits">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#advanced-traits")
}}>
   Advanced Traits
  </a>
 </h2>
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch10-02-traits.html#traits-defining-shared-behavior")
}}>
...
about Rust, we can get into the nitty-gritty.
 ...
 <h3 id="specifying-placeholder-types-in-trait-definitions-with-associated-types">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#specifying-placeholder-types-in-trait-definitions-with-associated-types")
}}>
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 19-12: The definition of the
   <code>
    Iterator
   </code>
   trait
...
definition that uses associated types, we can only choose what the type of
  ...
  will be once, because there can only be one
  <code>
...
and documenting the associated type in the API documentation is good practice.
 ...
 <h3 id="default-generic-type-parameters-and-operator-overloading">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#default-generic-type-parameters-and-operator-overloading")
}}>
...
&#125;</code></pre></pre>
 <p>
  <span class="caption">
   Listing 19-14: Implementing the
   <code>
    Add
   </code>
   trait to overload
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 19-15: Implementing the
   <code>
    Add
   </code>
   trait on
...
implementation code.
 ...
 <h3 id="fully-qualified-syntax-for-disambiguation-calling-methods-with-the-same-name">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#fully-qualified-syntax-for-disambiguation-calling-methods-with-the-same-name")
}}>
...
&#125;</code></pre></pre>
 <p>
  <span class="caption">
   Listing 19-18: Specifying which trait’s
   <code>
    fly
   </code>
   method we
...
we used in Listing 19-18 doesn’t help here; if we change
  <code>
   main
  </code>
  to the code in
...
&#125;</code></pre>
 <p>
  <span class="caption">
   Listing 19-20: Attempting to call the
   <code>
    baby_name
   </code>
   function from the
...
to identify which implementation you want to call.
 ...
 <h3 id="using-supertraits-to-require-one-traits-functionality-within-another-trait">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-supertraits-to-require-one-traits-functionality-within-another-trait")
}}>
...
<span class="boring">
 ...
 <p>
  <span class="caption">
   Listing 19-22: Implementing the
   <code>
    OutlinePrint
   </code>
   trait that
requires the functionality from
   <code>
    Display
   </code>
  </span>
...
it within an outline of asterisks.
 ...
 <h3 id="using-the-newtype-pattern-to-implement-external-traits-on-external-types">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-the-newtype-pattern-to-implement-external-traits-on-external-types")
}}>
...
around this restriction using the
  <em>
   newtype pattern
  </em>
...
implementing the
  ...
  trait (discussed in Chapter 15 in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch15-02-deref.html#treating-smart-pointers-like-regular-references-with-the-deref-trait")
}}>
   “Treating Smart
Pointers Like Regular References with the
   <code>
    Deref
   </code>
   Trait”
  ...
  to return
the inner type would be a solution. If we don’t want the
  <code>
   Wrapper
  </code>
  type to have
