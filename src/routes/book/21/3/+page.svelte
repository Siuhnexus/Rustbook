<main>
 <h2 id="advanced-functions-and-closures">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#advanced-functions-and-closures")
}}>
...
including function pointers and returning closures.
 ...
 <h3 id="function-pointers">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#function-pointers")
}}>
   Function Pointers
  </a>
 </h3>
...
coerce to the type
  ...
  (with a lowercase f), not to be confused with the
  <code>
   Fn
  </code>
  closure trait. The
...
that of closures, as shown in Listing 19-27, where we’ve defined a function
  ...
  that adds one to its parameter. The function
  <code>
   do_twice
  </code>
  takes two
...
in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch19-03-advanced-traits.html#advanced-traits")
}}>
   “Advanced Traits”
  </a>
  <!-- ignore -->
  section because
there are multiple functions available named
  <code>
   to_string
  </code>
...
library has implemented for any type that implements
  <code>
   Display
  </code>
  .
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch06-01-defining-an-enum.html#enum-values")
}}>
   “Enum values”
  </a>
  <!-- ignore -->
...
compile to the same code, so use whichever style is clearer to you.
 ...
 <h3 id="returning-closures">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#returning-closures")
}}>
   Returning Closures
  </a>
 </h3>
