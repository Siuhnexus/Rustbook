<main>
 <h2 id="to-panic-or-not-to-panic">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#to-panic-or-not-to-panic")
}}>
   To
   <code>
    panic!
...
way that’s appropriate for its situation, or it could decide that an
  <code>
   Err
  </code>
  value in this case is unrecoverable, so it can call
  <code>
   panic!
  </code>
  and turn your
recoverable error into an unrecoverable one. Therefore, returning
  <code>
   Result
  </code>
  is a
...
some general guidelines on how to decide whether to panic in library code.
 ...
 <h3 id="examples-prototype-code-and-tests">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#examples-prototype-code-and-tests")
}}>
...
understood that a call to a method like
  <code>
   unwrap
  </code>
...
that method isn’t the functionality under test. Because
  ...
  is how a test
is marked as a failure, calling
  <code>
   unwrap
  </code>
  or
...
happen.
 ...
 <h3 id="cases-in-which-you-have-more-information-than-the-compiler">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#cases-in-which-you-have-more-information-than-the-compiler")
}}>
   Cases in Which You Have More Information Than the Compiler
  </a>
 </h3>
 <p>
  It would also be appropriate to call
  <code>
   unwrap
  </code>
  or
...
other logic that ensures the
  ...
  value, but the logic
isn’t something the compiler understands. You’ll still have a
  <code>
   Result
  </code>
  value
...
that
  ...
  is a valid IP address, so it’s acceptable to use
  <code>
   expect
  </code>
...
of the
  ...
  method: we still get a
  <code>
   Result
  </code>
  value, and the compiler will
still make us handle the
  <code>
   Result
  </code>
  as if the
...
hardcoded into the program and therefore
  <em>
   did
  </em>
  have a possibility of failure,
we’d definitely want to handle the
  <code>
   Result
  </code>
...
the IP address from some other source instead.
 ...
 <h3 id="guidelines-for-error-handling">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#guidelines-for-error-handling")
}}>
...
end up in a bad state. In this context, a
  <em>
   bad state
  </em>
  is when some assumption,
...
work through an example of what we mean in the
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch17-03-oo-design-patterns.html#encoding-states-and-behavior-as-types")
}}>
    “Encoding States and Behavior
as Types”
   </a>
   <!-- ignore -->
...
insecure or harmful, the best choice might be to call
  <code>
   panic!
  </code>
  and alert the
...
limit. In these cases, returning a
  <code>
   Result
  </code>
...
This is the main reason the standard library will call
  <code>
   panic!
  </code>
  if you attempt
...
no reasonable way for calling code to recover; the calling
  <em>
   programmers
  </em>
  need
...
example, if you have a type rather than an
  <code>
   Option
  </code>
...
have
  ...
  . Your code then doesn’t have to handle
two cases for the
  <code>
   Some
  </code>
...
the parameter is never negative.
 ...
 <h3 id="creating-custom-types-for-validation">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-custom-types-for-validation")
}}>
...
about the problem, and calls
  <code>
   continue
  </code>
  to start the next iteration of the loop
and ask for another guess. After the
  <code>
   if
  </code>
...
comparisons between
  ...
  and the secret number knowing that
  <code>
   guess
  </code>
  is
...
confidently use the values they receive. Listing 9-13 shows one way to define a
  ...
  type that will only create an instance of
  <code>
   Guess
  </code>
  if the
...
values between 1 and 100
 ...
 <p>
  First we define a struct named
  <code>
   Guess
  </code>
...
holds an
 ...
 <p>
  Then we implement an associated function named
  <code>
   new
  </code>
  on
...
If
  ...
  doesn’t pass this test, we make a
  <code>
   panic!
  </code>
  call, which will alert
the programmer who is writing the calling code that they have a bug they need
to fix, because creating a
  <code>
   Guess
  </code>
  with a
...
of a
  ...
  does pass the test, we create a new
  <code>
   Guess
  </code>
  with its
...
to the
 ...
 <p>
  ...
  , doesn’t have any
other parameters, and returns an
  <code>
   i32
  </code>
...
a
  ...
  because its purpose is to get some data from its fields and return
it. This public method is necessary because the
  <code>
   value
  </code>
  field of the
  ...
  struct is private. It’s important that the
  <code>
   value
  </code>
...
the module
  ...
  , thereby ensuring there’s no way for a
  <code>
   Guess
  </code>
  to have a
...
then declare in its signature that it takes or returns a
  <code>
   Guess
  </code>
  rather than an
...
success or failure as well. Using
  <code>
   panic!
  </code>
  and
