<main>
 <h2 id="recoverable-errors-with-result">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#recoverable-errors-with-result")
}}>
...
terminating the process.
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch02-00-guessing-game-tutorial.html#handling-potential-failure-with-result")
}}>
...
detail in Chapter 10. What you need to know right now is that
  <code>
   T
  </code>
  represents
...
return may differ.
 ...
 <p>
  Let’s call a function that returns a
  <code>
   Result
  </code>
...
&#125;</code></pre></pre>
 ...
 <p>
  ...
  has been filled in by the implementation of
  <code>
   File::open
  </code>
...
success value,
  ...
  , which is a file handle. The type of
  <code>
   E
  </code>
  used in
...
information is exactly what the
  <code>
   Result
  </code>
  enum conveys.
...
brought into scope by the prelude, so we don’t need to specify
  <code>
   Result::
  </code>
  before the
 ...
 <p>
  ...
  , this code will return the inner
  <code>
   file
  </code>
  value out of
...
writing.
 ...
 <p>
  ...
  handles the case where we get an
  <code>
   Err
  </code>
  value from
  ...
  . In this example, we’ve chosen to call the
  <code>
   panic!
  </code>
  macro. If
...
code, we’ll see the following output from the
  <code>
   panic!
  </code>
  macro:
...
note: run with `RUST_BACKTRACE=1` environment variable to display a backtrace
 ...
 <h3 id="matching-on-different-errors">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#matching-on-different-errors")
}}>
   Matching on Different Errors
  </a>
 </h3>
 <p>
  The code in Listing 9-4 will
  <code>
   panic!
  </code>
  no matter why
...
and return the handle to the new file. If
  <code>
   File::open
  </code>
...
representing the different kinds of errors that might result from an
  <code>
   io
  </code>
  operation. The variant we want to use is
  <code>
   ErrorKind::NotFound
  </code>
  , which indicates
the file we’re trying to open doesn’t exist yet. So we match on
  ...
  , but we also have an inner match on
  <code>
   error.kind()
  </code>
  .
...
we try to create the file with
  ...
  could also fail, we need a second arm in the inner
  <code>
   match
  </code>
  expression. When the
...
the missing file error.
 ...
 <section aria-role="note" class="note">
  <h4 id="alternatives-to-using-match-with-resultt-e">
   <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#alternatives-to-using-match-with-resultt-e")
}}>
...
with many of the methods defined on
   <code>
    Result&lt;T, E&gt;
   </code>
...
9-5, this time using closures and the
   <code>
    unwrap_or_else
   </code>
   method:
...
after you’ve read Chapter 13, and look up the
   <code>
    unwrap_or_else
   </code>
   method in the
...
nested
 ...
 <h4 id="shortcuts-for-panic-on-error-unwrap-and-expect">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#shortcuts-for-panic-on-error-unwrap-and-expect")
}}>
...
communicate intent well. The
  <code>
   Result&lt;T, E&gt;
  </code>
  type has many helper methods
defined on it to do various, more specific tasks. The
  <code>
   unwrap
  </code>
  method is a
shortcut method implemented just like the
  <code>
   match
  </code>
...
call the
  ...
  macro for us. Here is an example of
  <code>
   unwrap
  </code>
  in action:
...
&#125;</code></pre></pre>
 <p>
  If we run this code without a
  <em>
   hello.txt
  </em>
  file, we’ll see an error message from
the
  <code>
   panic!
  </code>
...
called `Result::unwrap()` on an `Err` value: Os &#123; code: 2, kind: NotFound, message: "No such file or directory" &#125;
 ...
 <p>
  ...
  method lets us also choose the
  <code>
   panic!
  </code>
  error message.
...
the
  ...
  macro. The error message used by
  <code>
   expect
  </code>
  in its call to
  ...
  will be the parameter that we pass to
  <code>
   expect
  </code>
...
information to use in debugging.
 ...
 <h3 id="propagating-errors">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#propagating-errors")
}}>
   Propagating Errors
  </a>
 </h3>
...
value of the type
  ...
  , where the generic parameter
  <code>
   T
  </code>
  has been
filled in with the concrete type
  <code>
   String
  </code>
...
this function’s body that might fail: the
  <code>
   File::open
  </code>
  function and the
...
If
  ...
  succeeds, the file handle in the pattern variable
  <code>
   file
  </code>
  becomes the value in the mutable variable
  <code>
   username_file
  </code>
  and the function
...
the file handle in
  ...
  because it
might fail, even though
  <code>
   File::open
  </code>
  succeeded. So we need another
  <code>
   match
  </code>
  to
...
same way that we returned the error value in the
  <code>
   match
  </code>
  that handled the
return value of
 ...
 <p>
  ...
  value
that contains a username or an
  <code>
   Err
  </code>
...
question mark operator
 ...
 <h4 id="a-shortcut-for-propagating-errors-the--operator">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#a-shortcut-for-propagating-errors-the--operator")
}}>
...
as the
  ...
  expressions we defined to handle the
  <code>
   Result
  </code>
  values in Listing
9-6. If the value of the
  <code>
   Result
  </code>
  is an
...
reasons.
 ...
 <p>
  ...
  function in Listing
9-7 to return a custom error type named
  <code>
   OurError
  </code>
...
library provides the convenient
  <code>
   fs::read_to_string
  </code>
...
into that
  ...
  , and returns it. Of course, using
  <code>
   fs::read_to_string
  </code>
...
the longer way first.
 ...
 <h4 id="where-the--operator-can-be-used">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#where-the--operator-can-be-used")
}}>
   Where The
   <code>
    ?
...
as the
  ...
  value. The return type of the function has to be a
  <code>
   Result
  </code>
  so that
...
on an
  ...
  is similar to its behavior when called on a
  <code>
   Result&lt;T, E&gt;
  </code>
  :
...
character there, but it’s also possible that there isn’t. This code takes the
  ...
  string slice argument and calls the
  <code>
   lines
  </code>
...
from the iterator. If
  <code>
   text
  </code>
  is the empty string, this call to
  <code>
   next
  </code>
  will
...
return a
  ...
  value containing a string slice of the first line in
  <code>
   text
  </code>
  .
 ...
 <p>
  ...
  extracts the string slice, and we can call
  <code>
   chars
  </code>
...
function in one line. If we couldn’t use the
  ...
  , we’d
have to implement this logic using more method calls or a
  <code>
   match
  </code>
  expression.
...
from Listing 9-10, but we’ve changed the return type of
  <code>
   main
  </code>
  to be
...
Types”
  ...
  <!-- ignore -->
  section in Chapter 17. For now, you can
read
  <code>
   Box&lt;dyn Error&gt;
  </code>
  ...
  function with the error type
  <code>
   Box&lt;dyn Error&gt;
  </code>
  is allowed
because it allows any
  <code>
   Err
  </code>
...
more code that returns other errors is added to the body of
  <code>
   main
  </code>
  .
...
they exit: programs that exit successfully return the integer
  ...
  , and programs
that error return some integer other than
  <code>
   0
  </code>
...
executables to be compatible with this convention.
 ...
 <p>
  ...
  function may return any types that implement
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/process/trait.Termination.html")
}}>
   the
   <code>
...
documentation for more information on implementing the
  <code>
   Termination
  </code>
  trait for
