<main>
 <h2 id="refactoring-to-improve-modularity-and-error-handling">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#refactoring-to-improve-modularity-and-error-handling")
}}>
...
program grows, the number of separate tasks the
  <code>
   main
  </code>
...
one task.
 ...
 <p>
  ...
  are configuration variables to our program, variables like
  <code>
   contents
  </code>
  are used
to perform the program’s logic. The longer
  <code>
   main
  </code>
...
without specifying enough arguments, they’ll get an
  <code>
   index out of bounds
  </code>
  error
...
that will be meaningful to our end users.
 ...
 <h3 id="separation-of-concerns-for-binary-projects">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#separation-of-concerns-for-binary-projects")
}}>
...
program and
  ...
  handles all the logic of the task at hand. Because you
can’t test the
  <code>
   main
  </code>
  function directly, this structure lets you test all of
your program’s logic by moving it into functions in
  <em>
   lib.rs
  </em>
  . The code that
remains in
  ...
  will be small enough to verify its correctness by reading
it. Let’s rework our program by following this process.
 </p>
 <h4 id="extracting-the-argument-parser">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#extracting-the-argument-parser")
}}>
 ...
 <p>
  ...
  . Listing 12-5 shows the new start of
  <code>
   main
  </code>
  that calls a new
...
assigning the argument value at index 1 to the variable
  <code>
   query
  </code>
  and the
argument value at index 2 to the variable
  <code>
   file_path
  </code>
  within the
  ...
  function, we pass the whole vector to the
  <code>
   parse_config
  </code>
  function. The
...
goes in which variable and passes the values back to
  <code>
   main
  </code>
  . We still create
...
often, to help identify the cause of problems when they occur.
 ...
 <h4 id="grouping-configuration-values">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#grouping-configuration-values")
}}>
...
&#125;</code></pre>
 ...
 <p>
  We’ve added a struct named
  <code>
   Config
  </code>
  defined to have fields named
  <code>
   query
  </code>
  and
...
string slices that reference
  ...
  is the owner of
the argument values and is only letting the
  <code>
   parse_config
  </code>
  function borrow
them, which means we’d violate Rust’s borrowing rules if
  <code>
   Config
  </code>
  tried to take
ownership of the values in
  <code>
   args
  </code>
  .
 ...
 <p>
  There are a number of ways we could manage the
  <code>
   String
  </code>
  data; the easiest,
though somewhat inefficient, route is to call the
  <code>
   clone
  </code>
  method on the values.
This will make a full copy of the data for the
  <code>
   Config
  </code>
...
giving up a little performance to gain simplicity is a worthwhile trade-off.
 ...
 <section aria-role="note" class="note">
  <h3 id="the-trade-offs-of-using-clone">
   <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-trade-offs-of-using-clone")
}}>
...
ownership problems because of its runtime cost. In
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch13-00-functional-features.html")
}}>
    Chapter 13
   </a>
   <!-- ignore -->
...
the fields on the
 ...
 <p>
  Now our code more clearly conveys that
  <code>
   query
  </code>
  and
...
uses these values knows to find them in the
  <code>
   config
  </code>
...
named for their purpose.
 ...
 <h4 id="creating-a-constructor-for-config">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-constructor-for-config")
}}>
...
relationship should be conveyed in our code. We then added a
  <code>
   Config
  </code>
  struct to
name the related purpose of
  <code>
   query
  </code>
  and
...
values’ names as struct field names from the
  <code>
   parse_config
  </code>
  function.
 ...
 <p>
  So now that the purpose of the
  <code>
   parse_config
  </code>
...
changing
  ...
  , we’ll
be able to create instances of
  <code>
   Config
  </code>
  by calling
...
compiling this code again to make sure it works.
 ...
 <h3 id="fixing-the-error-handling">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#fixing-the-error-handling")
}}>
...
they should do instead. Let’s fix that now.
 ...
 <h4 id="improving-the-error-message">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#improving-the-error-message")
}}>
   Improving the Error Message
  </a>
 </h4>
 <p>
  In Listing 12-8, we add a check in the
  <code>
   new
  </code>
...
<span class="boring">
 ...
 <p>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch09-03-to-panic-or-not-to-panic.html#creating-custom-types-for-validation")
}}>
   the
   <code>
    Guess::new
...
a range of values here, we’re checking that the length of
  <code>
   args
  </code>
  is at least
...
will be
 ...
 <p>
  With these extra few lines of code in
  <code>
   new
  </code>
...
technique we used in Listing 9-13 isn’t the best one to use here: a call to
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch09-03-to-panic-or-not-to-panic.html#guidelines-for-error-handling")
}}>
   as discussed in Chapter 9
  </a>
  <!-- ignore -->
  . Instead,
we’ll use the other technique you learned about in Chapter 9—
  <a href="#" onclick={() => {
    // @ts-ignore
 ...
 <p>
  <a id="returning-a-result-from-new-instead-of-calling-panic">
  </a>
 </p>
 <h4 id="returning-a-result-instead-of-calling-panic">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#returning-a-result-instead-of-calling-panic")
}}>
   Returning a
   <code>
    Result
 ...
 <p>
  ...
  value that will contain a
  <code>
   Config
  </code>
  instance in
the successful case and will describe the problem in the error case. We’re also
going to change the function name from
  <code>
   new
  </code>
  to
...
problem. Then we can change
  ...
  variant into a more
practical error for our users without the surrounding text about
  <code>
   thread 'main'
  </code>
  and
...
to return a
  ...
  . Note that this won’t compile until we update
  <code>
   main
  </code>
  as
...
string literals that have the
 ...
 <p>
  ...
  when the user doesn’t pass enough arguments, we now return an
  <code>
   Err
  </code>
  value, and
...
process more cleanly in the error case.
 ...
 <!-- Old headings. Do not remove or links may break. -->
 <p>
  <a id="calling-confignew-and-handling-errors">
  </a>
 </p>
 <h4 id="calling-configbuild-and-handling-errors">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#calling-configbuild-and-handling-errors")
}}>
   Calling
   <code>
    Config::build
...
tool with a nonzero error code away from
  <code>
   panic!
  </code>
...
Using
  ...
  allows us to define some custom, non-
  <code>
   panic!
  </code>
  error
...
to
  ...
  : it returns the inner value that
  <code>
   Ok
  </code>
...
value is an
  ...
  value, this method calls the code in the
  <em>
   closure
  </em>
  , which is
an anonymous function we define and pass as an argument to
  <code>
   unwrap_or_else
  </code>
  .
We’ll cover closures in more detail in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch13-00-functional-features.html")
}}>
...
the
  ...
  that we added in Listing 12-9, to our closure in the argument
  <code>
   err
  </code>
  that
...
Problem parsing arguments: not enough arguments
 ...
 <h3 id="extracting-logic-from-main">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#extracting-logic-from-main")
}}>
   Extracting Logic from
   <code>
    main
   </code>
  </a>
...
the program’s logic. As we stated in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#separation-of-concerns-for-binary-projects")
}}>
...
inspection, and we’ll be able to write tests for all the other logic.
 ...
 <p>
  Listing 12-11 shows the extracted
  <code>
   run
  </code>
  function. For now, we’re just making
the small, incremental improvement of extracting the function. We’re still
defining the function in
  <em>
   src/main.rs
  </em>
  .
...
<span class="boring">
  ...
  <figcaption>
   Listing 12-11: Extracting a
   <code>
    run
   </code>
   function containing the rest of the program logic
  </figcaption>
 </figure>
 <p>
  The
  ...
  function now contains all the remaining logic from
  <code>
   main
  </code>
  , starting
...
argument.
 ...
 <h4 id="returning-errors-from-the-run-function">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#returning-errors-from-the-run-function")
}}>
   Returning Errors from the
   <code>
    run
   </code>
   Function
 ...
 <p>
  ...
  function, we can
improve the error handling, as we did with
  <code>
   Config::build
  </code>
  in Listing 12-9.
Instead of allowing the program to panic by calling
  <code>
   expect
  </code>
  , the
...
us further consolidate the logic around handling errors into
  <code>
   main
  </code>
  in a
...
returned the unit type,
 ...
 <p>
  For the error type, we used the
  <em>
   trait object
  </em>
  <code>
...
We’ll cover trait objects in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch17-00-oop.html")
}}>
   Chapter 17
  </a>
  <!-- ignore -->
...
may be of different types in different error cases. The
  <code>
   dyn
  </code>
  keyword is short
for
 ...
 <p>
  Second, we’ve removed the call to
  <code>
   expect
  </code>
  in favor of the
...
talked about in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch09-02-recoverable-errors-with-result.html#a-shortcut-for-propagating-errors-the--operator")
}}>
   Chapter 9
  </a>
  <!-- ignore -->
...
We’ve declared the
  ...
  in the signature,
which means we need to wrap the unit type value in the
  <code>
   Ok
  </code>
  value. This
  ...
  like this is
the idiomatic way to indicate that we’re calling
  <code>
   run
  </code>
...
To an admiring bog!
 ...
 <p>
  Rust tells us that our code ignored the
  <code>
   Result
  </code>
  value and the
...
have some error-handling code here! Let’s rectify that problem now.
 ...
 <h4 id="handling-errors-returned-from-run-in-main">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#handling-errors-returned-from-run-in-main")
}}>
   Handling Errors Returned from
   <code>
    run
   </code>
   in
...
<span class="boring">
 ...
 <p>
  ...
  function
doesn’t return a value that we want to
  <code>
   unwrap
  </code>
...
both cases: we print the error and exit.
 ...
 <h3 id="splitting-code-into-a-library-crate">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#splitting-code-into-a-library-crate")
}}>
 ...
 <p>
  ...
  file and put some code into the
  <em>
   src/lib.rs
  </em>
  file. That way, we
can test the code and have a
  <em>
   src/main.rs
  </em>
 ...
 <p>
  Let’s move all the code that isn’t in the
  <code>
   main
  </code>
  function from
 ...
 <p>
  ...
  should have the signatures shown in Listing 12-13
(we’ve omitted the bodies of the functions for brevity). Note that this won’t
compile until we modify
  <em>
   src/main.rs
  </em>
  in Listing 12-14.
...
a public API we can test!
 ...
 <p>
  Now we need to bring the code we moved to
  <em>
   src/lib.rs
  </em>
...
&#125;</code></pre>
 ...
 <p>
  ...
  type from the
library crate into the binary crate’s scope, and we prefix the
  <code>
   run
  </code>
  function
with our crate name. Now all the functionality should be connected and should
work. Run the program with
  <code>
   cargo run
  </code>
...
modular. Almost all of our work will be done in
  <em>
   src/lib.rs
  </em>
  from here on out.
