<main>
 <h2 id="accepting-command-line-arguments">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#accepting-command-line-arguments")
}}>
...
for our program rather than for
  <code>
   cargo
  </code>
...
a file to search in, like so:
 ...
 <p>
  Right now, the program generated by
  <code>
   cargo new
  </code>
  cannot process arguments we
give it. Some existing libraries on
  <a href="#" onclick={() => {
    // @ts-ignore
...
just learning this concept, let’s implement this capability ourselves.
 ...
 <h3 id="reading-the-argument-values">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#reading-the-argument-values")
}}>
...
produce a series of values, and we can call the
  <code>
   collect
  </code>
...
the iterator produces.
 ...
 <p>
  The code in Listing 12-1 allows your
  <code>
   minigrep
  </code>
...
can use its
  ...
  function is
nested in two levels of modules. As we discussed in
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch07-04-bringing-paths-into-scope-with-the-use-keyword.html#creating-idiomatic-use-paths")
}}>
   Chapter
7
  </a>
...
from
  ...
  . It’s also less ambiguous than adding
  <code>
   use std::env::args
  </code>
  and
then calling the function with just
  <code>
   args
  </code>
  , because
...
mistaken for a function that’s defined in the current module.
 ...
 <section aria-role="note" class="note">
  <h3 id="the-args-function-and-invalid-unicode">
   <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#the-args-function-and-invalid-unicode")
}}>
    The
    <code>
     args
...
by the iterator. We can use the
  <code>
   collect
  </code>
...
chapter, we’ll ignore it and save only the two arguments we need.
 ...
 <h3 id="saving-the-argument-values-in-variables">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#saving-the-argument-values-in-variables")
}}>
...
reference to the first argument in the variable
  <code>
   query
  </code>
  . The second argument
...
working as we intend. Let’s run this program again with the arguments
  <code>
   test
  </code>
  and
