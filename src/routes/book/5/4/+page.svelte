<main>
 <h2 id="control-flow">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#control-flow")
}}>
   Control Flow
  </a>
 </h2>
...
the flow of execution of Rust code are
 ...
 <h3 id="if-expressions">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#if-expressions")
}}>
   <code>
    if
   </code>
...
&#125;</code></pre></pre>
 <p>
  ...
  expressions start with the keyword
  <code>
   if
  </code>
  , followed by a condition. In
this case, the condition checks whether or not the variable
  <code>
   number
  </code>
  has a
...
associated with the conditions in
  <code>
   if
  </code>
...
just like the arms in
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch02-00-guessing-game-tutorial.html#comparing-the-guess-to-the-secret-number")
}}>
   “Comparing
...
the condition is
  ...
  , the program will just skip the
  <code>
   if
  </code>
  block and move on
...
condition was true
 ...
 <p>
  Let’s try changing the value of
  <code>
   number
  </code>
...
&#125;</code></pre>
 <p>
  ...
  condition evaluates to a value of
  <code>
   3
  </code>
...
error: could not compile `branches` (bin "branches") due to 1 previous error
 ...
 <p>
  The error indicates that Rust expected a
  <code>
   bool
  </code>
...
convert non-Boolean types to a Boolean. You must be explicit and always provide
  ...
  with a Boolean as its condition. If we want the
  <code>
   if
  </code>
  code block to run
only when a number is not equal to
  <code>
   0
  </code>
...
&#125;</code></pre></pre>
 ...
 <h4 id="handling-multiple-conditions-with-else-if">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#handling-multiple-conditions-with-else-if")
}}>
 ...
 <p>
  You can use multiple conditions by combining
  <code>
   if
  </code>
  and
...
the first body for which the condition evaluates to
  <code>
   true
  </code>
  . Note that even
though 6 is divisible by 2, we don’t see the output
  <code>
...
Rust branching construct called
  <code>
   match
  </code>
  for these cases.
 ...
 <h4 id="using-if-in-a-let-statement">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-if-in-a-let-statement")
}}>
   Using
   <code>
    if
 ...
 <p>
  ...
  is an expression, we can use it on the right side of a
  <code>
   let
  </code>
...
&#125;</code></pre></pre>
 ...
 <p>
  ...
  variable will be bound to a value based on the outcome of the
  <code>
   if
  </code>
...
values that have the potential to be results from each arm of the
  <code>
   if
  </code>
  must be
the same type; in Listing 3-2, the results of both the
  <code>
   if
  </code>
  arm and the
...
have a single type, and Rust needs to know at compile time what type the
  ...
  variable is, definitively. Knowing the type of
  <code>
   number
  </code>
  lets the
compiler verify the type is valid everywhere we use
  <code>
   number
  </code>
  . Rust wouldn’t be
able to do that if the type of
  <code>
   number
  </code>
...
if it had to keep track of multiple hypothetical types for any variable.
 ...
 <h3 id="repetition-with-loops">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#repetition-with-loops")
}}>
...
Rust provides several
  <em>
   loops
  </em>
  , which will run through the code inside the loop
body to the end and then start immediately back at the beginning. To experiment
with loops, let’s make a new project called
  <em>
   loops
  </em>
  .
 ...
 <h4 id="repeating-code-with-loop">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#repeating-code-with-loop")
}}>
   Repeating Code with
   <code>
    loop
...
until we stop the program manually. Most terminals support the keyboard shortcut
  ...
  to interrupt a program that is stuck in a continual
loop. Give it a try:
 </p>
 <!-- manual-regeneration
...
executing the loop. Recall that we did this in the guessing game in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch02-00-guessing-game-tutorial.html#quitting-after-a-correct-guess")
}}>
...
next iteration.
 ...
 <h4 id="returning-values-from-loops">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#returning-values-from-loops")
}}>
...
this, you can add the value you want returned after the
  <code>
   break
  </code>
  expression you
...
the loop. On every iteration of the loop, we add
  <code>
   1
  </code>
  to the
...
semicolon to end the statement that assigns the value to
  <code>
   result
  </code>
  . Finally, we
...
loop,
 ...
 <h4 id="loop-labels-to-disambiguate-between-multiple-loops">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#loop-labels-to-disambiguate-between-multiple-loops")
}}>
...
loop at that point. You can optionally specify a
  <em>
   loop label
  </em>
  on a loop that
...
&#125;</code></pre></pre>
 <p>
  The outer loop has the label
  <code>
   'counting_up
  </code>
  , and it will count up from 0 to 2.
The inner loop without a label counts down from 10 to 9. The first
  <code>
   break
  </code>
  that
...
End count = 2
 ...
 <h4 id="conditional-loops-with-while">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#conditional-loops-with-while")
}}>
   Conditional Loops with
   <code>
    while
   </code>
  </a>
...
condition is
  ...
  , the loop runs. When the condition ceases to be
  <code>
   true
  </code>
  , the
...
like this using a combination of
  <code>
   loop
  </code>
  ,
...
evaluates to
 ...
 <h4 id="looping-through-a-collection-with-for">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#looping-through-a-collection-with-for")
}}>
...
for each item in a collection. A
  <code>
   for
  </code>
...
certain number of times, as in the countdown example that used a
  <code>
   while
  </code>
  loop
in Listing 3-3, most Rustaceans would use a
  <code>
   for
  </code>
...
and compound data types, functions, comments,
  <code>
   if
  </code>
