<main>
 <h2 id="storing-utf-8-encoded-text-with-strings">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#storing-utf-8-encoded-text-with-strings")
}}>
...
creating, updating, and reading. We’ll also discuss the ways in which
  ...
  is different from the other collections, namely how indexing into a
  <code>
   String
  </code>
  is
complicated by the differences between how people and computers interpret
 ...
 <h3 id="what-is-a-string">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#what-is-a-string")
}}>
   What Is a String?
  </a>
 </h3>
 <p>
  We’ll first define what we mean by the term
  <em>
   string
  </em>
...
referring to either the
  ...
  types, not just one
of those types. Although this section is largely about
  <code>
   String
  </code>
  , both types are
used heavily in Rust’s standard library, and both
  <code>
   String
  </code>
  and string slices
are UTF-8 encoded.
 ...
 <h3 id="creating-a-new-string">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-new-string")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  <span class="caption">
   Listing 8-11: Creating a new, empty
   <code>
    String
   </code>
  </span>
...
redundant, but they all have their place! In this case,
  <code>
   String::from
  </code>
  and
...
strings
 ...
 <h3 id="updating-a-string">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#updating-a-string")
}}>
   Updating a String
  </a>
 </h3>
...
use the
 ...
 <h4 id="appending-to-a-string-with-push_str-and-push">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#appending-to-a-string-with-push_str-and-push")
}}>
...
parameter. For example, in the code in Listing 8-16, we want to be able to use
  ...
  after appending its contents to
  <code>
   s1
  </code>
  .
...
using
 ...
 <h4 id="concatenation-with-the--operator-or-the-format-macro">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#concatenation-with-the--operator-or-the-format-macro")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
 ...
 <p>
  ...
  is no longer
valid after the addition, and the reason we used a reference to
  <code>
   s2
  </code>
  , has to do
...
bits of the
 ...
 <p>
  ...
  , meaning that we’re adding a
  <em>
   reference
  </em>
  of the second
string to the first string. This is because of the
  <code>
   s
  </code>
  parameter in the
  ...
  values together. But wait—the type of
  <code>
   &amp;s2
  </code>
  is
  ...
  , as
specified in the second parameter to
  <code>
   add
  </code>
...
this statement actually takes ownership of
  <code>
   s1
  </code>
...
more complicated ways, we can instead use the
  <code>
   format!
  </code>
  macro:
...
<span class="boring">&#125;</span></code></pre></pre>
 <p>
  ...
  with the contents. The version of the code using
  <code>
   format!
  </code>
  is much
easier to read, and the code generated by the
  <code>
   format!
  </code>
...
so that this call doesn’t take ownership of any of its parameters.
 ...
 <h3 id="indexing-into-strings">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#indexing-into-strings")
}}>
...
if you try to access parts of a
  <code>
   String
  </code>
...
memory.
 ...
 <h4 id="internal-representation">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#internal-representation")
}}>
...
begins with the capital Cyrillic letter
  <em>
   Ze
  </em>
  , not the number 3):
...
if the string contains only Latin letters: if
  ...
  were valid code
that returned the byte value, it would return
  <code>
   104
  </code>
  , not
...
at all and prevents misunderstandings early in the development process.
 ...
 <h4 id="bytes-and-scalar-values-and-grapheme-clusters-oh-my">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#bytes-and-scalar-values-and-grapheme-clusters-oh-my")
}}>
...
clusters (the closest thing to what we would call
  <em>
   letters
  </em>
  ).
...
them as Unicode scalar values, which are what Rust’s
  <code>
   char
  </code>
  type is, those
...
(O(1)). But it isn’t possible to guarantee that performance with a
  <code>
   String
  </code>
  ,
...
index to determine how many valid characters there were.
 ...
 <h3 id="slicing-strings">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#slicing-strings")
}}>
   Slicing Strings
  </a>
 </h3>
...
so can crash your program.
 ...
 <h3 id="methods-for-iterating-over-strings">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#methods-for-iterating-over-strings")
}}>
...
functionality you need.
 ...
 <h3 id="strings-are-not-so-simple">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#strings-are-not-so-simple")
}}>
