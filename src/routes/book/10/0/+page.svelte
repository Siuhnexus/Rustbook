<main>
 <h2 id="storing-lists-of-values-with-vectors">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#storing-lists-of-values-with-vectors")
}}>
   Storing Lists of Values with Vectors
  </a>
 </h2>
 <p>
  The first collection type we’ll look at is
  <code>
   Vec&lt;T&gt;
  </code>
  , also known as a
...
lines of text in a file or the prices of items in a shopping cart.
 ...
 <h3 id="creating-a-new-vector">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-a-new-vector")
}}>
...
annotation. Rust conveniently provides the
  <code>
   vec!
  </code>
...
new vector that holds the values you give it. Listing 8-2 creates a new
  ...
  because that’s the default integer type, as we discussed in the
  <a href="#" onclick={() => {
    // @ts-ignore
...
values
 ...
 <p>
  Because we’ve given initial
  <code>
   i32
  </code>
  values, Rust can infer that the type of
  <code>
   v
  </code>
  is
...
to modify a vector.
 ...
 <h3 id="updating-a-vector">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#updating-a-vector")
}}>
   Updating a Vector
  </a>
 </h3>
...
we don’t need the
 ...
 <h3 id="reading-elements-of-vectors">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#reading-elements-of-vectors")
}}>
...
<span class="boring">&#125;</span></code></pre></pre>
 ...
 <p>
  Note a few details here. We use the index value of
  <code>
   2
  </code>
...
because vectors are indexed by number, starting at zero. Using
  ...
  method with the index passed as an argument, we get an
  <code>
   Option&lt;&amp;T&gt;
  </code>
  that we can
...
number that’s too large and the program gets a
  <code>
   None
  </code>
...
ending up in that situation.
 ...
 <section aria-role="note" class="note">
  <p>
   ...
   <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../nomicon/vec/vec.html")
}}>
    “The
Rustonomicon”
   </a>
 ...
 <h3 id="iterating-over-the-values-in-a-vector">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#iterating-over-the-values-in-a-vector")
}}>
...
iterating over the elements using a
   <code>
    for
   </code>
   loop
...
in order to make changes to all the elements. The
  <code>
   for
  </code>
  loop in Listing 8-8
...
elements in a vector
 ...
 <p>
  ...
  dereference operator to get to the value in
  <code>
   i
  </code>
  ...
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch15-02-deref.html#following-the-pointer-to-the-value-with-the-dereference-operator")
}}>
   “Following the
...
whole vector.
 ...
 <h3 id="using-an-enum-to-store-multiple-types">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-an-enum-to-store-multiple-types")
}}>
...
the vector. Using an enum plus a
  <code>
   match
  </code>
...
to review
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("../std/vec/struct.Vec.html")
}}>
   the API documentation
  </a>
  <!-- ignore -->
  for all of the many
useful methods defined on
  <code>
   Vec&lt;T&gt;
  </code>
...
addition to
 ...
 <h3 id="dropping-a-vector-drops-its-elements">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#dropping-a-vector-drops-its-elements")
}}>
...
valid.
 ...
 <p>
  Let’s move on to the next collection type:
  <code>
   String
  </code>
  !
