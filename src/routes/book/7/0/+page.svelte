<main>
 <h2 id="defining-and-instantiating-structs">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#defining-and-instantiating-structs")
}}>
...
the pieces of data, which we call
  <em>
   fields
  </em>
  . For example, Listing 5-1 shows a
...
&#125;</code></pre></pre>
  <figcaption>
   Listing 5-2: Creating an instance of the
   <code>
    User
   </code>
   struct
...
access this user’s email address, we use
  <code>
   user1.email
  </code>
  . If the instance is
...
&#125;</code></pre></pre>
  <figcaption>
   Listing 5-3: Changing the value in the
   <code>
    email
   </code>
   field of a
...
implicitly return that new instance.
 ...
 <p>
  ...
  instance with
the given email and username. The
  <code>
   active
  </code>
...
<span class="boring">
  ...
  <figcaption>
   ...
   function that takes an email and username and returns a
   <code>
    User
   </code>
   instance
...
fields, but having to repeat the
  <code>
   email
  </code>
  and
...
would get even more annoying. Luckily, there’s a convenient shorthand!
 ...
 <p>
  <a id="using-the-field-init-shorthand-when-variables-and-fields-have-the-same-name">
  </a>
 </p>
 <h3 id="using-the-field-init-shorthand">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-the-field-init-shorthand")
}}>
...
<span class="boring">
  ...
  <figcaption>
   ...
   function that uses field init shorthand because the
   <code>
    username
   </code>
   and
...
than
 ...
 <h3 id="creating-instances-from-other-instances-with-struct-update-syntax">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#creating-instances-from-other-instances-with-struct-update-syntax")
}}>
...
the values from another instance, but changes some. You can do this using
 ...
 <p>
  ...
  regularly, without the update syntax. We set a new value for
  <code>
   email
  </code>
  but
otherwise use the same values from
  <code>
   user1
  </code>
...
&#125;</code></pre></pre>
  <figcaption>
   ...
   instance using all but one of the values from
   <code>
    user1
   </code>
  </figcaption>
...
&#125;</code></pre></pre>
  <figcaption>
   ...
   instance but to use the rest of the values from
   <code>
    user1
   </code>
  </figcaption>
...
it moves the data, just as we saw in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch04-01-what-is-ownership.html#variables-and-data-interacting-with-move")
}}>
   “Variables and Data Interacting with
Move”
  </a>
  <!-- ignore -->
...
implement the
  ...
  trait, so the behavior we discussed in the
  <a href="#" onclick={() => {
    // @ts-ignore
    window.externallink("ch04-01-what-is-ownership.html#stack-only-data-copy")
}}>
   “Stack-Only
Data: Copy”
  </a>
  <!-- ignore -->
  section would apply. We can still use
  <code>
   user1.email
  </code>
  in this example, since its value was
  <em>
   not
  </em>
  moved out.
 ...
 <h3 id="using-tuple-structs-without-named-fields-to-create-different-types">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#using-tuple-structs-without-named-fields-to-create-different-types")
}}>
...
example, a function that takes a parameter of type
  <code>
   Color
  </code>
  cannot take a
  ...
  as an argument, even though both types are made up of three
  <code>
   i32
  </code>
...
by the index to access an individual value.
 ...
 <h3 id="unit-like-structs-without-any-fields">
  <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#unit-like-structs-without-any-fields")
}}>
...
implement them on any type, including unit-like structs.
 ...
 <section aria-role="note" class="note">
  <h3 id="ownership-of-struct-data">
   <a class="header" href="#" onclick={() => {
    // @ts-ignore
    window.externallink("#ownership-of-struct-data")
}}>
  ...
  <p>
   ...
   struct definition in Listing 5-1, we used the owned
   <code>
    String
   </code>
...
else, but to do so requires the use of
   <em>
    lifetimes
   </em>
   , a Rust feature that we’ll
